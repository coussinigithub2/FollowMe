...
-- ⚠ float_wnd_set_visible N'EXISTE PAS dans FlyWithLua NG.
-- Pour cacher : float_wnd_destroy()   Pour montrer : float_wnd_create()
-- La fenêtre principale est créée À LA DEMANDE (clic sur holder).

-- Holder (petit bouton FM cliquable) — toujours présent
fm_holder = float_wnd_create(30, 30, 2, true)
...
-- Fenêtre principale — démarre NIL, créée à la demande
fm_window = nil

function show_window()
    window_first_access = true
    window_is_open      = true
    fm_window = float_wnd_create(320, 540, 2, true)
...
function hide_window()
    if fm_window ~= nil then
        float_wnd_destroy(fm_window)
        fm_window      = nil
...
do_every_frame([[
    ...
    -- Toggle fenêtre principale (demande du holder)
    -- ⚠ destroy/create au lieu de set_visible (non supporté dans FlyWithLua NG)
