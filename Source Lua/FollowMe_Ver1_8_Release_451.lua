...
function plot_position( in_act_dist )
  ...
  if car_sign == 0 or curr_node >= #t_node - 2 then 
      ...
      if curr_node ~= #t_node then
          ...
          -- VER1.8 : for back-taxi, only stop at the very last node (runway threshold)
          -- For normal departures, stop at first hotzone as before
          if depart_arrive == 1 and t_node[curr_node].hotzone == "1" and curr_node >= #t_node - 2 then
              if not is_backtaxi or curr_node == #t_node then
                  car_sign = 1
