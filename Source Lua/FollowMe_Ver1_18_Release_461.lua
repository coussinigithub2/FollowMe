...
--    VER1.17 Coussini 2026: Crash fix — airport change caused X-Plane access violation.
--                           FlyWithLua restarts the Lua engine on each airport/plane change
--                           (LUA_RUN increments). The previous XPLMRegisterDataAccessor calls
--                           for tire_steer_deg/tire_rotation_angle_deg/fm_anim_sign left
--                           dangling XPLM pointers that X-Plane read after the Lua context
--                           was destroyed → crash. Fix: native tire datarefs now use
--                           XPLMFindDataRef + XPLMSetDatavf each frame (no registration).
--                           fm/anim/sign registered only on LUA_RUN==1, found via
--                           XPLMFindDataRef on subsequent runs. dataref_float_value now
--                           correctly filled with steering/tire_rotate each frame so
--                           XPLMInstanceSetPosition wheel animations work properly.
--    VER1.18 Coussini 2026: Wrong runway threshold bug — at airports where runways
--                           physically intersect (e.g. KSYR RWY 10/28 crosses 15/33),
--                           the A* route to RWY 28 passed through nodes 126/134 on
--                           RWY 15/33 before reaching node 135 on RWY 10/28. The
--                           hotzone_node_cnt limit (>2) triggered a break at node 126
--                           (the RWY 33 threshold), stopping the car on the wrong runway.
--                           Fix: (1) runway-type nodes are counted only if they belong to
--                           depart_runway (checked via segment IDs), ignoring cross-runway
--                           nodes; (2) the break is never applied on runway_endpt_node
--                           (the A* destination) to ensure the final correct node is
--                           always included regardless of the count.
--    ---------------------------------------------------------------------------------
--    ---------------------------------------------------------------------------------
...
local runway_endpt_node = -1   -- VER1.18 : nœud d'entrée piste (endpt A*), partagé avec process_possible_routes
    ...
    if depart_arrive == 1 then
        for l_index = 1, #t_runway do
          if t_runway[l_index].ID == depart_runway then
              ...
              runway_endpt_node = l_endpt_node   -- VER1.18 : partagé avec process_possible_routes
              break
          end
        end
...
function process_possible_routes()
    ...
    for routenode_cnt = 1, #t_route_nodes do 
        ...
        if depart_arrive == 1 and routenode_cnt >= l_last_taxiway then  
            if t_taxinode[l_node+1].Type == "hotzone" or t_taxinode[l_node+1].Type == "runway" then
                -- VER1.18 : pour un nœud de type runway, ne compter QUE s'il a au moins
                -- un segment appartenant à depart_runway. Cela évite que le passage par
                -- une intersection de pistes (ex KSYR : 15/33 croise 10/28) ne déclenche
                -- le break prématurément sur le mauvais seuil.
                local l_count_this = true
                if t_taxinode[l_node+1].Type == "runway" then
                    l_count_this = false
                    for l_seg_str in t_taxinode[l_node+1].Segment:gmatch("[^,]+") do
                        ...
                        if l_sidx and t_segment[l_sidx] and t_segment[l_sidx].Type == "runway" then
                            for l_part in (t_segment[l_sidx].ID or ""):gmatch("[^/]+") do
                                if l_part == depart_runway then l_count_this = true; break end
                            end
                        end
                        if l_count_this then break end
                    end
                end
                if l_count_this then
                    l_hotzone_node_cnt = l_hotzone_node_cnt + 1
                end
            end    
        end    
        -- VER1.18 : ne jamais breaker sur le nœud destination lui-même (runway_endpt_node)
        -- Cas KSYR : le nœud 135 (seuil 28) est le 3e nœud sur la bonne piste → cnt=3 > 2
        -- mais c'est le bon nœud final, on doit l'inclure.
        if l_hotzone_node_cnt > 2 and l_node ~= runway_endpt_node then break end   
    ...
    -- VER1.15 : back-taxi -- follow the actual runway nodes from entry to threshold.
    -- Instead of a single straight-line virtual node (VER1.9), we walk the runway
    -- segments from the entry node toward the threshold, inserting each intermediate
    -- runway node into t_node. The car follows the centreline exactly.
    -- VER1.16 : Centreline projection -- t_node[#t_node] is the A* last node: the
    -- taxiway/runway junction node. It sits on the EDGE of the runway, NOT on the
    -- centreline. Going straight from there to the threshold causes the diagonal
    -- curve into the grass seen in the simulator screenshot.
    -- Fix: project t_node[#t_node] perpendicularly onto the runway centreline and
    -- REPLACE it in-place. All intermediate runway walk nodes are also projected.
    -- Result: the car arrives at the exact centre of the runway, then walks straight
    -- to the threshold. No curve, no field. Universal for all airports.
    if is_backtaxi and depart_arrive == 1 and #t_node > 0 then
        ...
        -- VER1.16 : save original edge position, then project onto centreline
        ...
        if l_offset > 2 and l_proj_to_thr > 10 then
            ...
            logMsg("FollowMe VER1.16 : RWY " .. depart_runway ..
        ...
        -- Find entry node ID (matched against original edge position before projection)
        local l_entry_nid = -1
        for l_si = 1, #t_segment do
            if t_segment[l_si] and t_segment[l_si].Type == "runway" then
                local l_nx = t_taxinode[t_segment[l_si].Node1 + 1].x
                local l_nz = t_taxinode[t_segment[l_si].Node1 + 1].z
                local _, l_d = heading_n_dist(l_edge_x, l_edge_z, l_nx, l_nz)
                if l_d < 5 then l_entry_nid = t_segment[l_si].Node1; break end
                l_nx = t_taxinode[t_segment[l_si].Node2 + 1].x
                l_nz = t_taxinode[t_segment[l_si].Node2 + 1].z
                _, l_d = heading_n_dist(l_edge_x, l_edge_z, l_nx, l_nz)
                if l_d < 5 then l_entry_nid = t_segment[l_si].Node2; break end
            end
        end

        -- Walk runway nodes toward threshold; project each onto centreline
        if l_entry_nid >= 0 then
            local l_cur = l_entry_nid
            local l_visited = {}
            local l_max_steps = 30
            local l_step = 0
            l_visited[l_cur] = true

            while l_step < l_max_steps do
                l_step = l_step + 1
                local l_best_next = -1
                local l_best_d = 99999
                local _, l_cur_d = heading_n_dist(
                    t_taxinode[l_cur+1].x, t_taxinode[l_cur+1].z, l_rwy_x, l_rwy_z)

                for l_si = 1, #t_segment do
                    if t_segment[l_si] and t_segment[l_si].Type == "runway" then
                        local l_other = -1
                        if t_segment[l_si].Node1 == l_cur then
                            l_other = t_segment[l_si].Node2
                        elseif t_segment[l_si].Node2 == l_cur then
                            l_other = t_segment[l_si].Node1
                        end
                        if l_other >= 0 and not l_visited[l_other] then
                            local _, l_nd = heading_n_dist(
                                t_taxinode[l_other+1].x, t_taxinode[l_other+1].z,
                                l_rwy_x, l_rwy_z)
                            if l_nd < l_cur_d and l_nd < l_best_d then
                                l_best_d = l_nd
                                l_best_next = l_other
                            end
                        end
                ...
                if l_best_next < 0 then break end
                l_visited[l_best_next] = true
                l_cur = l_best_next

                local l_last = #t_node
                local l_nx = t_taxinode[l_cur+1].x
                local l_nz = t_taxinode[l_cur+1].z
                -- Project intermediate node onto centreline
                local l_wt = (l_nx - l_far_x) * l_ux + (l_nz - l_far_z) * l_uz
                l_nx = l_far_x + l_wt * l_ux
                l_nz = l_far_z + l_wt * l_uz
                local l_ny = probe_y(l_nx, 0, l_nz)
                t_node[l_last].heading, t_node[l_last].dist =
                    heading_n_dist(t_node[l_last].x, t_node[l_last].z, l_nx, l_nz)
                t_node[l_last + 1] = {}
                t_node[l_last + 1].x       = l_nx
                t_node[l_last + 1].y       = l_ny
                t_node[l_last + 1].z       = l_nz
        ...
        logMsg("FollowMe VER1.16 : back-taxi RWY " .. depart_runway ..
               " total_nodes=" .. (#t_node))
    end
