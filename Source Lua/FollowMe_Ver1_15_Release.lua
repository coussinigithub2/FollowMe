...
function process_possible_routes()
    ...
    if is_backtaxi and depart_arrive == 1 and #t_node > 0 then
        ...
        -- Find the entry node (last node of t_node = where D/taxiway meets the runway)
        local l_entry_nid = -1
        for l_si = 1, #t_segment do
            if t_segment[l_si] and t_segment[l_si].Type == "runway" then
                ...
                local _, l_d = heading_n_dist(t_node[#t_node].x, t_node[#t_node].z, l_nx, l_nz)
                ...
                _, l_d = heading_n_dist(t_node[#t_node].x, t_node[#t_node].z, l_nx, l_nz)
                if l_d < 5 then l_entry_nid = t_segment[l_si].Node2; break end
            end
        end

        -- Walk runway nodes from entry toward threshold (greedy: pick neighbour
        -- that is closer to the threshold than current, via runway segments only)
        if l_entry_nid >= 0 then
            ...
            while l_step < l_max_steps do
                ...
                -- Find runway segment neighbour closer to threshold
                for l_si = 1, #t_segment do
                    if t_segment[l_si] and t_segment[l_si].Type == "runway" then
                        local l_other = -1
                ...
                if l_best_next < 0 then break end  -- no progress toward threshold

                l_visited[l_best_next] = true
                l_cur = l_best_next

                -- Insert this runway node into t_node
                local l_last = #t_node
                local l_nx = t_taxinode[l_cur+1].x
                local l_ny = t_taxinode[l_cur+1].y
        ...
        -- Final node: exact GPS threshold (in case last runway node is not exactly at threshold)
        local l_last = #t_node
        ...
        logMsg("FollowMe VER1.15 : back-taxi RWY " .. depart_runway ..
               " — " .. (#t_node) .. " nodes along runway centreline to threshold")
    end
