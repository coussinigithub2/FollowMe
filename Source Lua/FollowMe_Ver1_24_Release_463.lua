...
function transverse(in_startnode, in_endnode, in_heading)
	...
	if t_taxinode[in_endnode+1].f_value ~= nil then
		...
		logMsg("FollowMe VER1.24 : RAW A* ROUTE" ..
			" AIRPORT=" .. curr_ICAO ..
			" FROM=" .. l_from_label ..
			" TO=" .. l_to_label)
...
if l_rwy_idx > 0 and t_runway[l_rwy_idx].Pair ~= nil then
	...
	if t_runway[l_pair_idx] ~= nil then
		...
		logMsg("FollowMe VER1.24 : RWY " .. depart_runway ..
			" centreline from pair idx=" .. l_pair_idx ..
			" ID=" .. (t_runway[l_pair_idx].ID or "?") ..
			" lat=" .. string.format("%.6f", t_runway[l_pair_idx].Lat or 0) ..
...
"(lat=" .. string.format("%.6f", l_gps_lat) ..
" lon=" .. string.format("%.6f", l_gps_lon) ..
...
logMsg("FollowMe VER1.24 : DRIVE ROUTE" ..
	" AIRPORT=" .. curr_ICAO ..
	" FROM=" .. l_from_lbl ..
	" TO=" .. l_to_lbl)
