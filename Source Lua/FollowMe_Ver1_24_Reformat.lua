	...
	if l_dist_to_plane > 150 then
		...
		logMsg(
			"FollowMe VER1.12 : car spawned behind aircraft (startpt was "
				.. string.format("%.0f", l_dist_to_plane)
				.. "m away)"
		)
	end
...
function chk_line_of_sight(
	...
	if
		l_heading_to_target >= l_left_arc
		and l_heading_to_target <= l_left_arc + shooter_left_angle + shooter_right_angle
	then
		l_within_sight = true
	elseif
		l_heading_to_target <= l_right_arc
		and l_heading_to_target >= l_right_arc - (shooter_left_angle + shooter_right_angle)
	then
		l_within_sight = true
	end
...
function manage_car_motion()
	...
	if remaining_dist_leg > 0 then
		...
		if t_node[curr_node + 1].dir ~= nil and t_node[curr_node + 1].speed ~= nil and curr_node + 1 ~= #t_node then
			if
				turning_is_active == 0
				and t_node[curr_node + 1].dir ~= 0
				and car_speed > t_node[curr_node + 1].speed
			then
...
function plot_position(in_act_dist)
	...
	if turning_is_active == 2 then
		...
		car_x = t_node[curr_node + 1].rot2_x
			+ math.sin(math.rad(l_heading_from_center)) * t_node[curr_node + 1].ref_rot_radius
		car_z = t_node[curr_node + 1].rot2_z
			+ math.cos(math.rad(l_heading_from_center)) * t_node[curr_node + 1].ref_rot_radius * -1
	...
	if car_sign == 0 or curr_node >= #t_node - 2 then
		if curr_node ~= #t_node then
			if
				remaining_dist_leg < avg_dist_from_plane
				and t_node[curr_node + 1].dir ~= nil
				and t_node[curr_node + 1].AoC < 160
			then
...
function determine_dir_of_turn(in_head1, in_head2, in_dist)
	...
	if t_node[curr_node + 1].dir ~= 0 then
		...
		if l_AoC < 90 then
			...
			then
				t_node[curr_node + 1].dist_b4_turn = t_node[curr_node + 1].dist
					- (min_rot_radius + car_rear_wheel_to_ref)
...
function apply_1206_filter()
	...
	logMsg(
		"FollowMe VER1.23 : apply_1206_filter() removed "
			...
			.. l_seg_count
			.. " orphaned segments at "
			.. (curr_ICAO ~= "" and curr_ICAO or "unknown")
	)
end
	...
	for l_idx = 1, #t_taxinode do
		if t_taxinode[l_idx].Type == "runway" then
			...
			if l_has_own_segs and not l_rwy_nodes[l_node_id] then
				...
				for l_seg = 1, #t_segment do
					if
						(t_segment[l_seg].Node1 == l_node_id or t_segment[l_seg].Node2 == l_node_id)
						and t_segment[l_seg].Type ~= "runway"
					then
						l_has_twy = true
						break
					end
	...
	if l_chosen == -1 then
		...
		logMsg(
			"FollowMe VER1.15 : RWY "
				...
				.. l_chosen
				.. " ("
				.. string.format("%.1f", l_chosen_dist)
				.. "m)"
		)
	else
		logMsg(
...
function handle_plugin_window()
		...
		if l_teleport_dist >= 1000 then
			...
			logMsg(
				"FollowMe VER1.12 : teleport detected ("
					.. string.format("%.0f", l_teleport_dist)
					.. "m jump) - full_reset()"
			)
		end
	end
	...
	if FM_car_active == false and get_from_SimBrief == true and fm_gear1_gnd == 1 and holder_wnd then
		if
			(prev_taxi_light ~= fm_taxi_light and fm_taxi_light == 1)
			or (prev_beacon_light ~= fm_beacon_light and fm_beacon_light == 1)
		then
	...
	if
		...
		and fm_gear1_gnd == 1
		and fm_gear2_gnd == 1
		and fm_gnd_spd > 10.288
		and fm_run_time > speed_warn_time
	then
	...
	if
		FM_car_active == false
		and flightstart == 9999
		and (fm_gear1_gnd == 1 and fm_gear2_gnd == 1)
		and ground_time == 0
		and fm_taxi_light == 1
		and fm_gnd_spd < 15
	then
		...
		if taxiway_network == "" then
			if random_gate == true or arrival_gate > 0 then
				if arrival_gate == 0 then
					...
					if l_err == "1" then
						play_text = "No suitable gate for this aircraft. Randomly assigned to gate "
							.. t_gate[arrival_gate].ID
					elseif l_err == "2" then
	...
	if holder_wnd then
		if
			MOUSE_X >= SCREEN_WIDTH - Holder_len
			and MOUSE_X <= SCREEN_WIDTH
			and MOUSE_Y >= SCREEN_HIGHT - (Win_Y + Holder_len)
			and MOUSE_Y <= SCREEN_HIGHT - Win_Y
		then
	...
	if followme_wnd ~= nil then
		...
		if not l_over_holder then
			if
				MOUSE_X >= SCREEN_WIDTH - 400 - Holder_len
				and MOUSE_X <= SCREEN_WIDTH - Holder_len
				and MOUSE_Y >= SCREEN_HIGHT - (Win_Y + 400)
				and MOUSE_Y <= SCREEN_HIGHT - Win_Y
			then
...
function build_window(wnd, x, y)
	...
	if combo_filter_list == true then
		...
		if imgui.Begin("##combo_filter", nil, l_flags) then
			...
			for i = 1, #t_gate do
				if
					string.match(t_gate[i].Types, Aircraft_Type) == Aircraft_Type
					or (Aircraft_Type == "0" and string.match(t_gate[i].Types, "7"))
				then
...
function transverse(in_startnode, in_endnode, in_heading)
	...
	while l_curr_node ~= in_endnode and l_curr_node ~= -1 do
		for l_string in t_taxinode[l_curr_node + 1].Segment:gmatch("[^,]+") do
			...
			l_pass = false
			if
				t_segment[l_segment_idx].Type ~= "runway"
				or (t_segment[l_segment_idx].Type == "runway" and not impose_restriction_chk)
			then
			...
			if t_runway[l_pair_idx] ~= nil then
				...
				logMsg(
					"FollowMe VER1.24 : RWY "
						...
						.. " lon="
						.. string.format("%.6f", t_runway[l_pair_idx].Lon or 0)
				)
			end
		end
		...
		if l_offset > 2 and l_proj_to_thr > 10 then
			...
			logMsg(
				"FollowMe VER1.17 : RWY "
					...
					.. "m"
					.. " proj_to_threshold="
					.. string.format("%.0f", l_proj_to_thr)
					.. "m"
			)
		else
			t_node[#t_node].hotzone = "1"
...
function compute_tangent_dist(in_heading, in_x, in_z, in_idx)
	...
	if
		(
			(l_tangent_x >= l_node1_x and l_tangent_x <= l_node2_x)
			or (l_tangent_x >= l_node2_x and l_tangent_x <= l_node1_x)
		)
		or (
			(l_tangent_z >= l_node1_z and l_tangent_z <= l_node2_z)
			or (l_tangent_z >= l_node2_z and l_tangent_z <= l_node1_z)
		)
	then
...
function compute_intersection(in_type, in_heading, in_x, in_z, in_idx)
	...
	if
		(
			(l_intersect_x >= l_node1_x and l_intersect_x <= l_node2_x)
			or (l_intersect_x >= l_node2_x and l_intersect_x <= l_node1_x)
		)
		and (
			(l_intersect_z >= l_node1_z and l_intersect_z <= l_node2_z)
			or (l_intersect_z >= l_node2_z and l_intersect_z <= l_node1_z)
		)
	then
	else
		return false, 0, 0, 0
...
function add_new_taxinode_segment(in_segment_index, in_x, in_z, in_intersect_dist)
	...
	if string.find(t_taxinode[t_segment[in_segment_index].Node1 + 1].Segment, ",") then
		...
		t_taxinode[t_segment[in_segment_index].Node1 + 1].Segment = t_taxinode[t_segment[in_segment_index].Node1 + 1].Segment
			.. ","
			.. tostring(l_new_segment)
	...
	if string.find(t_taxinode[t_segment[in_segment_index].Node2 + 1].Segment, ",") then
		...
		t_taxinode[t_segment[in_segment_index].Node2 + 1].Segment = t_taxinode[t_segment[in_segment_index].Node2 + 1].Segment
			.. ","
			.. tostring(l_new_segment)
...
function auto_assign_gate()
	...
	for l_index = 1, #t_gate do
		if
			string.match(t_gate[l_index].Types, Aircraft_Type) == Aircraft_Type
			or (Aircraft_Type == "0" and string.match(t_gate[l_index].Types, "7"))
		then
