...
--    VER1.13 Coussini 2026: Virtual intersection nodes — for each taxiway endpoint near a runway
--                           segment with no direct runway connection, a virtual node is created at
--                           the geometric intersection point and connected to both the taxiway and
--                           the runway on each side. This closes the network: the car can never
--                           leave the paved surface regardless of airport apt.dat quality.
--    ---------------------------------------------------------------------------------
--    ---------------------------------------------------------------------------------
...
function build_virtual_intersections()
  -- VER1.13 : For each taxiway endpoint that is close to a runway segment
  -- but has no direct runway neighbour, create a virtual intersection node.
  -- This ensures the car can never leave the taxiway/runway network ("no fields").
  -- Thresholds are excluded (seuil de piste) — only mid-runway intersections.

  local THRESHOLD = 50   -- max metres between taxiway endpoint and runway segment
  local l_virtual_count = 0

  -- Collect runway segments
  local l_rwy_segs = {}
  for l_idx = 1, #t_segment do
      if t_segment[l_idx].Type == "runway" then
          l_rwy_segs[#l_rwy_segs+1] = l_idx
      end
  end

  -- Collect taxiway endpoints (nodes that appear in taxiway segments)
  local l_twy_endpoints = {}
  for l_idx = 1, #t_segment do
      if t_segment[l_idx].Type ~= "runway" then
          l_twy_endpoints[t_segment[l_idx].Node1] = true
          l_twy_endpoints[t_segment[l_idx].Node2] = true
      end
  end

  -- For each taxiway endpoint, check if it already has a runway neighbour
  for l_nid, _ in pairs(l_twy_endpoints) do
      local l_has_rwy = false
      local l_node_seg_str = t_taxinode[l_nid+1] and t_taxinode[l_nid+1].Segment or ""
      for l_seg_str in l_node_seg_str:gmatch("[^,]+") do
          local l_sidx = tonumber(l_seg_str)
          if l_sidx and t_segment[l_sidx] and t_segment[l_sidx].Type == "runway" then
              l_has_rwy = true; break
          end
      end
      if l_has_rwy then goto continue_node end

      -- No runway neighbour — check proximity to runway segments
      local l_px = t_taxinode[l_nid+1].x
      local l_pz = t_taxinode[l_nid+1].z
      local l_best_dist = THRESHOLD + 1
      local l_best_seg  = 0
      local l_best_cx, l_best_cz, l_best_t = 0, 0, 0

      for _, l_sidx in ipairs(l_rwy_segs) do
          local l_n1 = t_segment[l_sidx].Node1
          local l_n2 = t_segment[l_sidx].Node2
          local l_ax = t_taxinode[l_n1+1].x; local l_az = t_taxinode[l_n1+1].z
          local l_bx = t_taxinode[l_n2+1].x; local l_bz = t_taxinode[l_n2+1].z
          local l_dx = l_bx-l_ax; local l_dz = l_bz-l_az
          local l_len2 = l_dx*l_dx + l_dz*l_dz
          if l_len2 < 1 then goto continue_seg end
          local l_t = ((l_px-l_ax)*l_dx + (l_pz-l_az)*l_dz) / l_len2
          if l_t < 0.02 or l_t > 0.98 then goto continue_seg end  -- exclude threshold ends
          local l_cx = l_ax + l_t*l_dx; local l_cz = l_az + l_t*l_dz
          local l_d  = math.sqrt((l_px-l_cx)^2 + (l_pz-l_cz)^2)
          if l_d < l_best_dist then
              l_best_dist = l_d; l_best_seg = l_sidx
              l_best_cx = l_cx; l_best_cz = l_cz; l_best_t = l_t
          end
          ::continue_seg::
      end

      if l_best_seg > 0 then
          -- Create virtual node at the projection point
          local l_vid = #t_taxinode  -- new 0-based node id = current length
          t_taxinode[l_vid+1] = {}
          -- Get altitude from one of the runway endpoints
          local l_n1 = t_segment[l_best_seg].Node1
          local l_n2 = t_segment[l_best_seg].Node2
          local l_vx, l_vz = l_best_cx, l_best_cz
          local l_vy = (t_taxinode[l_n1+1].y + t_taxinode[l_n2+1].y) / 2
          local l_vlat, l_vlon, _ = local_to_latlon(l_vx, l_vy, l_vz)
          t_taxinode[l_vid+1].x   = l_vx
          t_taxinode[l_vid+1].y   = l_vy
          t_taxinode[l_vid+1].z   = l_vz
          t_taxinode[l_vid+1].Lat = l_vlat
          t_taxinode[l_vid+1].Lon = l_vlon
          t_taxinode[l_vid+1].Type    = "runway"
          t_taxinode[l_vid+1].Runway  = ""
          t_taxinode[l_vid+1].Segment = ""
          t_taxinode[l_vid+1].f_value = nil; t_taxinode[l_vid+1].g_value = nil
          t_taxinode[l_vid+1].h_value = nil; t_taxinode[l_vid+1].parent  = nil
          t_taxinode[l_vid+1].cost    = nil; t_taxinode[l_vid+1].heading = nil

          -- Create 3 new segments: taxiway→virtual, virtual→rwy_n1, virtual→rwy_n2
          local l_rwy_nm  = t_segment[l_best_seg].ID
          local l_rwy_typ = "runway"

          -- Helper to add a segment
          local function add_seg(na, nb, typ, nm)
              local l_s = #t_segment + 1
              t_segment[l_s] = {}
              t_segment[l_s].Node1   = na
              t_segment[l_s].Node2   = nb
              t_segment[l_s].Dir     = "twoway"
              t_segment[l_s].Type    = typ
              t_segment[l_s].ID      = nm
              t_segment[l_s].Hotzone = ""
              local l_hd, l_di = heading_n_dist(t_taxinode[na+1].x, t_taxinode[na+1].z,
                                                 t_taxinode[nb+1].x, t_taxinode[nb+1].z)
              t_segment[l_s].Heading = l_hd; t_segment[l_s].Dist = l_di
              -- Register in taxinode Segment strings
              local function reg(n, si)
                  if t_taxinode[n+1].Segment == "" then
                      t_taxinode[n+1].Segment = tostring(si)
                  else
                      t_taxinode[n+1].Segment = t_taxinode[n+1].Segment .. "," .. tostring(si)
                  end
              end
              reg(na, l_s); reg(nb, l_s)
          end

          add_seg(l_nid, l_vid, "taxiway_B", l_rwy_nm)
          add_seg(l_vid, l_n1,  l_rwy_typ,  l_rwy_nm)
          add_seg(l_vid, l_n2,  l_rwy_typ,  l_rwy_nm)

          t_virtual_intersections_added = (t_virtual_intersections_added or 0) + 1
          l_virtual_count = l_virtual_count + 1
          logMsg("FollowMe VER1.13 : virtual intersection node " .. l_vid ..
                 " added (taxiway→runway " .. l_rwy_nm .. " dist=" ..
                 string.format("%.1f", l_best_dist) .. "m)")
      end

      ::continue_node::
  end

  if l_virtual_count > 0 then
      logMsg("FollowMe VER1.13 : " .. l_virtual_count .. " virtual intersection node(s) added for " .. curr_ICAO)
  end
end
  ...
  if combo_filter_list == true then
    ...
    if imgui.Begin("##combo_filter", nil, l_flags) then
      ...
      if #t_suitable_gates == 0 then  
          l_err = "-17"
          for i = 1, #t_gate  do t_suitable_gates[#t_suitable_gates+1] = i end    
      end
