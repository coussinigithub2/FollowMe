...
function start_car()
	...
	if l_dist_to_plane > 150 then
		...
		logMsg("FollowMe VER1.12 : car spawned behind aircraft (startpt was " .. string.format("%.0f", l_dist_to_plane) .. "m away)")
	end
...
function chk_line_of_sight(
	shooter_heading, shooter_left_angle, shooter_right_angle, shooter_x, shooter_z, target_x, target_z
)
...
function plot_position(in_act_dist)
	...
	if turning_is_active == 0 then
		...
		if l_remaining_leg_dist <= 0 then
			...
			if t_node[curr_node + 1].dir ~= 0 and curr_node + 1 ~= # t_node then
				turning_is_active = 1
				t_node[curr_node + 1].rot_x, t_node[curr_node + 1].rot_z = CoR_coordinates_using_car_ref(
					car_x, car_z, l_head1, t_node[curr_node + 1].radius, t_node[curr_node + 1].dir)
			else
	...
	if turning_is_active == 1 then
		if t_node[curr_node + 1].head1_exit == nil then
			l_head2, l_dist = heading_n_dist(
				t_node[curr_node + 1].rot_x, t_node[curr_node + 1].rot_z, t_node[curr_node + 2].x, t_node[curr_node + 2].z)
		...
		l_heading_from_center = minus_delta_clockwise(
			car_body_heading, 90 - t_node[curr_node + 1].angle_rear_to_ref, t_node[curr_node + 1].dir)
		...
		if turning_is_active == 2 then
			...
			t_node[curr_node + 1].rot2_x, t_node[curr_node + 1].rot2_z = CoR_coordinates_using_car_ref(
				car_x, car_z, car_body_heading, t_node[curr_node + 1].radius, t_node[curr_node + 1].dir * - 1)
			l_head2, l_dist = heading_n_dist(
				t_node[curr_node + 1].rot2_x, t_node[curr_node + 1].rot2_z, t_node[curr_node + 2].x, t_node[curr_node + 2].z)
			l_AoR = math.deg(math.acos(t_node[curr_node + 1].radius / l_dist))
	...
	if turning_is_active == 2 then
		...
		l_heading_from_center = minus_delta_clockwise(
			car_body_heading, 90 - t_node[curr_node + 1].angle_rear_to_ref, t_node[curr_node + 1].dir * - 1)
	...
	if car_sign == 0 or curr_node >= # t_node - 2 then
		if curr_node ~= # t_node then
			...
			if depart_arrive ~= 1 then
				local l_gate_in_sight, l_to_gate_heading, _ = chk_line_of_sight(
					car_body_heading, 45, 45, car_x, car_z, t_gate[arrival_gate].x, t_gate[arrival_gate].z)
...
function load_object()
	...
	if car_type_fmcar == "Ferrari" or (car_type_fmcar == "Auto" and l_auto_sel == 1) then
		XPLM.XPLMLoadObjectAsync(
			syspath .. "Resources/default scenery/airport scenery/Dynamic_Vehicles/crew_car_ferrari.obj", function(inObject, inRefcon)
	...
	elseif car_type_fmcar == "Van" or (car_type_fmcar == "Auto" and l_auto_sel == 2) then
		XPLM.XPLMLoadObjectAsync(SCRIPT_DIRECTORY .. "follow_me/objects/fm_van.obj", function(inObject, inRefcon)
			obj_instance[0] = XPLM.XPLMCreateInstance(inObject, datarefs_addr)
			objref = inObject
		end, inRefcon)
		car_weight = 1700
	...
	elseif car_type_fmcar == "Truck" or (car_type_fmcar == "Auto" and l_auto_sel == 3) then
		XPLM.XPLMLoadObjectAsync(SCRIPT_DIRECTORY .. "follow_me/objects/fm_truck.obj", function(inObject, inRefcon)
			obj_instance[0] = XPLM.XPLMCreateInstance(inObject, datarefs_addr)
			objref = inObject
		end, inRefcon)
		car_weight = 2000
	...
	XPLM.XPLMLoadObjectAsync(SCRIPT_DIRECTORY .. "follow_me/objects/signboard.obj", function(inObject, inRefcon)
...
function load_path()
	if FM_car_active and show_path and # t_node > 0 then
		XPLM.XPLMLoadObjectAsync(
			SCRIPT_DIRECTORY .. "follow_me/objects/pushpin_yellow.obj", function(inObject, inRefcon)
			for i = 0, # t_node - 1 do
...
function load_rampstart()
	if rampstart_instance[0] == nil then
		XPLM.XPLMLoadObjectAsync(
			SCRIPT_DIRECTORY .. "follow_me/objects/diamond_marker.obj", function(inObject, inRefcon)
...
function register_dataref()
	...
	if LUA_RUN == 1 then
		dr_sign = XPLM.XPLMRegisterDataAccessor("fm/anim/sign", 2, 0, NULL, NULL, function(inRefcon)
			return car_sign
							...
							for l_line2 in l_new_lines:gmatch("[^\r\n]+") do
								if l_not_first_line == false then
									...
									if string.find(l_line2, "^1%s") or l_line2 == "99" then
										table.sort(t_gate, function(a, b)
											return a.ID < b.ID
										end)
...
function decipher_runway(in_str)
	...
	t_runway[i] = {}
	t_runway[i + 1] = {}
	t_runway[i].ID, l_str1, l_str2, t_runway[i + 1].ID, l_str3, l_str4 = string.match(
		in_str, "100 %s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*" .. "([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*" .. "[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*" .. "([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*" .. "(.*)")
...
function decipher_taxisegment(in_str)
	...
	t_segment[i].Hotzone = ""
	t_segment[i].Heading, t_segment[i].Dist = heading_n_dist(
		t_taxinode[t_segment[i].Node1 + 1].x, t_taxinode[t_segment[i].Node1 + 1].z, t_taxinode[t_segment[i].Node2 + 1].x, t_taxinode[t_segment[i].Node2 + 1].z)
...
function apply_1206_filter()
	...
	logMsg("FollowMe VER1.23 : apply_1206_filter() removed " .. l_count .. " vehicle-only nodes and " .. l_seg_count .. " orphaned segments at " .. (curr_ICAO ~= "" and curr_ICAO or "unknown"))
end
...
function match_runway(in_runway_idx)
	...
	for l_idx = 1, # t_taxinode do
		if t_taxinode[l_idx].Type == "runway" then
			...
			if l_has_own_segs and not l_rwy_nodes[l_node_id] then
				...
				for l_seg = 1, # t_segment do
					if (t_segment[l_seg].Node1 == l_node_id or t_segment[l_seg].Node2 == l_node_id) and t_segment[l_seg].Type ~= "runway" then
						l_has_twy = true
						break
					end
				end
	...
	if l_chosen == - 1 then
		...
		logMsg("FollowMe VER1.15 : RWY " .. l_rwy_id .. " - no taxiway-connected node found, using closest node " .. l_chosen .. " (" .. string.format("%.1f", l_chosen_dist) .. "m)")
	else
		logMsg("FollowMe VER1.15 : RWY " .. l_rwy_id .. " -> node " .. l_chosen .. " (" .. string.format("%.1f", l_chosen_dist) .. "m from threshold)")
	end
...
function handle_plugin_window()
	...
	if prev_plane_x ~= 0 then
		...
		if l_teleport_dist >= 1000 then
			full_reset()
			logMsg("FollowMe VER1.12 : teleport detected (" .. string.format("%.0f", l_teleport_dist) .. "m jump) - full_reset()")
		end
	end
	...
	if FM_car_active == false and get_from_SimBrief == true and fm_gear1_gnd == 1 and holder_wnd then
		if (prev_taxi_light ~= fm_taxi_light and fm_taxi_light == 1) or (prev_beacon_light ~= fm_beacon_light and fm_beacon_light == 1) then
	...
	if speed_limiter and FM_car_active and fm_gear1_gnd == 1 and fm_gear2_gnd == 1 and fm_gnd_spd > 10.288 and fm_run_time > speed_warn_time then
		play_sound(snd_keep_speed)
		speed_warn_time = fm_run_time + 15
	end

	if FM_car_active == false and flightstart == 9999 and (fm_gear1_gnd == 1 and fm_gear2_gnd == 1) and ground_time == 0 and fm_taxi_light == 1 and fm_gnd_spd < 15 then
		get_airport_elements()
		if taxiway_network == "" then
	...
	if holder_wnd then
		if MOUSE_X >= SCREEN_WIDTH - Holder_len and MOUSE_X <= SCREEN_WIDTH and MOUSE_Y >= SCREEN_HIGHT - (Win_Y + Holder_len) and MOUSE_Y <= SCREEN_HIGHT - Win_Y then
	...
	if followme_wnd ~= nil then
		local l_over_holder = (
			MOUSE_X >= SCREEN_WIDTH - Holder_len and MOUSE_X <= SCREEN_WIDTH and MOUSE_Y >= SCREEN_HIGHT - (Win_Y + Holder_len) and MOUSE_Y <= SCREEN_HIGHT - Win_Y)
		if not l_over_holder then
			if MOUSE_X >= SCREEN_WIDTH - 400 - Holder_len and MOUSE_X <= SCREEN_WIDTH - Holder_len and MOUSE_Y >= SCREEN_HIGHT - (Win_Y + 400) and MOUSE_Y <= SCREEN_HIGHT - Win_Y then
...
function build_window(wnd, x, y)
	...
	local l_flags = bit.bor(
		imgui.constant.WindowFlags.NoTitleBar, imgui.constant.WindowFlags.NoResize, imgui.constant.WindowFlags.NoMove, imgui.constant.WindowFlags.HorizontalScrollbar, imgui.constant.WindowFlags.NoSavedSettings)
	...
	imgui.PushStyleColor(imgui.constant.Col.Text, 4287137928)
	...
	if get_from_SimBrief and sb_fetch_status == "OK" and sb_dest_icao ~= "" then
		...
		imgui.PushStyleColor(imgui.constant.Col.Text, 4284900966)
	...
	if combo_filter_list == true then
		...
		if imgui.Begin("##combo_filter", nil, l_flags) then
			...
			t_suitable_gates = {}
			for i = 1, # t_gate do
				if string.match(t_gate[i].Types, Aircraft_Type) == Aircraft_Type or (Aircraft_Type == "0" and string.match(t_gate[i].Types, "7")) then
	...
	if FM_car_active then
		...
		local l_spd_color = (speed_limiter and l_plane_kts > 20) and 4278190335 or 4278255360
		imgui.PushStyleColor(imgui.constant.Col.Text, l_spd_color)
		imgui.TextUnformatted(string.format("%.1f kts", l_plane_kts))
		imgui.PopStyleColor()
	else
		imgui.PushStyleColor(imgui.constant.Col.Text, 4282664004)
		imgui.TextUnformatted("---")
		imgui.PopStyleColor()
	end
	imgui.SameLine()
	...
	if FM_car_active then
		...
		imgui.PushStyleColor(imgui.constant.Col.Text, 4291611852)
	...
	for i = 1, 3 do
		...
		if i == 1 then
			...
			imgui.PushStyleColor(imgui.constant.Col.Text, 4285098345)
		end
...
function determine_possible_routes()
	...
	if depart_gate ~= 0 then
		l_found, l_startpt_node, l_startpt_x, l_startpt_z = determine_pos_on_segment(
			t_gate[depart_gate].Heading, t_gate[depart_gate].x, t_gate[depart_gate].z, t_gate[depart_gate].Ramptype)
	else
		...
		l_found, l_startpt_node, l_startpt_x, l_startpt_z = determine_pos_on_segment(fm_plane_head, l_adj_fm_plane_x, l_adj_fm_plane_z, "tie_down")
	...
	if depart_arrive == 1 then
		...
		l_found, l_endpt_node, l_endpt_x, l_endpt_z = determine_pos_on_segment(
			t_gate[arrival_gate].Heading, t_gate[arrival_gate].x, t_gate[arrival_gate].z, "gate")
		if l_found == false then
			return "-13"
		end
	end
	...
	for l_index = 1, # t_taxinode do
		_, t_taxinode[l_index].h_value = heading_n_dist(
			t_taxinode[l_index].x, t_taxinode[l_index].z, t_taxinode[l_endpt_node + 1].x, t_taxinode[l_endpt_node + 1].z)
	end
...
function transverse(in_startnode, in_endnode, in_heading)
	...
	if t_taxinode[in_endnode + 1].f_value ~= nil then
		...
		logMsg("FollowMe VER1.24 : RAW A* ROUTE" .. " AIRPORT=" .. curr_ICAO .. " FROM=" .. l_from_label .. " TO=" .. l_to_label)
...
function process_possible_routes()
	...
	for routenode_cnt = 1, # t_route_nodes do
		...
		if l_index > 1 then
			l_new_head, l_new_dist = heading_n_dist(
				t_node[l_index - 1].x, t_node[l_index - 1].z, t_taxinode[l_node + 1].x, t_taxinode[l_node + 1].z)
		end
		t_node[l_index] = {}
	...
	for l_index = 1, # t_node - 2 do
		if t_node[l_index].dist < l_min_dist_btw_nodes then
			t_node[l_index + 1].x, t_node[l_index + 1].z = coordinates_of_adjusted_ref(
				t_node[l_index + 1].x, t_node[l_index + 1].z, 0, (l_min_dist_btw_nodes - t_node[l_index].dist), t_node[l_index].heading)
			t_node[l_index].dist = l_min_dist_btw_nodes
			t_node[l_index + 1].heading, t_node[l_index + 1].dist = heading_n_dist(
				t_node[l_index + 1].x, t_node[l_index + 1].z, t_node[l_index + 2].x, t_node[l_index + 2].z)
		end
	end
	...
	if depart_arrive == 1 and # t_node > 0 then
		...
		if l_rwy_idx > 0 and t_runway[l_rwy_idx].Pair ~= nil then
			...
			if t_runway[l_pair_idx] ~= nil then
				...
				logMsg("FollowMe VER1.24 : RWY " .. depart_runway .. " centreline from pair idx=" .. l_pair_idx .. " ID=" .. (t_runway[l_pair_idx].ID or "?") .. " lat=" .. string.format("%.6f", t_runway[l_pair_idx].Lat or 0) .. " lon=" .. string.format("%.6f", t_runway[l_pair_idx].Lon or 0))
			end
		end
		...
		if l_offset > 2 and l_proj_to_thr > 10 then
			...
			if # t_node >= 2 then
				t_node[# t_node - 1].heading, t_node[# t_node - 1].dist = heading_n_dist(t_node[# t_node - 1].x, t_node[# t_node - 1].z, l_proj_x, l_proj_z)
			end
			logMsg("FollowMe VER1.17 : RWY " .. depart_runway .. " centreline_projection lateral_offset=" .. string.format("%.1f", l_offset) .. "m" .. " proj_to_threshold=" .. string.format("%.0f", l_proj_to_thr) .. "m")
		else
			t_node[# t_node].hotzone = "1"
			logMsg("FollowMe VER1.17 : RWY " .. depart_runway .. " already_on_centreline offset=" .. string.format("%.1f", l_offset) .. "m")
		end

		local l_last = # t_node
		local _, l_gap = heading_n_dist(t_node[l_last].x, t_node[l_last].z, l_rwy_x, l_rwy_z)
		if l_gap > 5 then
			t_node[l_last].heading, t_node[l_last].dist = heading_n_dist(t_node[l_last].x, t_node[l_last].z, l_rwy_x, l_rwy_z)
			t_node[l_last + 1] = {}
	...
	if # t_node > 0 then
		...
		for l_ni = 1, # t_node do
			...
			l_node_list = l_node_list .. l_ni .. l_hz .. "(lat=" .. string.format("%.6f", l_gps_lat) .. " lon=" .. string.format("%.6f", l_gps_lon) .. " x=" .. string.format("%.1f", t_node[l_ni].x or 0) .. " z=" .. string.format("%.1f", t_node[l_ni].z or 0) .. " hdg=" .. string.format("%.0f", t_node[l_ni].heading or 0) .. " d=" .. string.format("%.0f", t_node[l_ni].dist or 0) .. "m) "
		end
		logMsg("FollowMe VER1.24 : DRIVE ROUTE" .. " AIRPORT=" .. curr_ICAO .. " FROM=" .. l_from_lbl .. " TO=" .. l_to_lbl)
	...
	if l_decision == 0 then
		if in_type == "gate" or in_type == "misc" or in_type == "tie_down" or in_type == "hangar" then
			...
			if l_best_nid >= 0 then
				logMsg("FollowMe VER1.15 : fallback to nearest connected node " .. l_best_nid .. " (" .. string.format("%.0f", l_best_d) .. "m)")
...
function compute_tangent_dist(in_heading, in_x, in_z, in_idx)
	...
	if ((l_tangent_x >= l_node1_x and l_tangent_x <= l_node2_x) or (l_tangent_x >= l_node2_x and l_tangent_x <= l_node1_x)) or ((l_tangent_z >= l_node1_z and l_tangent_z <= l_node2_z) or (l_tangent_z >= l_node2_z and l_tangent_z <= l_node1_z)) then
...
function compute_intersection(in_type, in_heading, in_x, in_z, in_idx)
	...
	if ((l_intersect_x >= l_node1_x and l_intersect_x <= l_node2_x) or (l_intersect_x >= l_node2_x and l_intersect_x <= l_node1_x)) and ((l_intersect_z >= l_node1_z and l_intersect_z <= l_node2_z) or (l_intersect_z >= l_node2_z and l_intersect_z <= l_node1_z)) then
	else
		return false, 0, 0, 0
	end
...
function add_new_taxinode_segment(in_segment_index, in_x, in_z, in_intersect_dist)
	...
	if string.find(t_taxinode[t_segment[in_segment_index].Node1 + 1].Segment, ",") then
		...
		t_segment[l_new_segment].Heading, t_segment[l_new_segment].Dist = heading_n_dist(
			t_taxinode[t_segment[in_segment_index].Node1 + 1].x, t_taxinode[t_segment[in_segment_index].Node1 + 1].z, in_x, in_z)
	...
	if string.find(t_taxinode[t_segment[in_segment_index].Node2 + 1].Segment, ",") then
		...
		t_segment[l_new_segment].Hotzone = t_segment[in_segment_index].Hotzone
		t_segment[l_new_segment].Heading, t_segment[l_new_segment].Dist = heading_n_dist(
			in_x, in_z, t_taxinode[t_segment[in_segment_index].Node2 + 1].x, t_taxinode[t_segment[in_segment_index].Node2 + 1].z)
...
function auto_assign_gate()
	...
	for l_index = 1, # t_gate do
		if string.match(t_gate[l_index].Types, Aircraft_Type) == Aircraft_Type or (Aircraft_Type == "0" and string.match(t_gate[l_index].Types, "7")) then
...
function build_holder(wnd, x, y)
	...
	local l_color = l_in_flight and 4284900966 or 4294967295
	local l_text = "FM"
	...
	if l_in_flight then
		local l_r = 10
		imgui.DrawList_AddLine(l_cx + l_r, l_cy - l_r, l_cx - l_r, l_cy + l_r, 4278190335, 2)
	end
