...
--    VER1.14 Coussini 2026: Three network integrity fixes:
--                           (1) Runway crossing nodes — when two runway segments from
--                           different runways cross geometrically, a virtual node is
--                           inserted at the exact intersection and connected to all 4
--                           half-segments. Without this, A* can route "through the fields"
--                           at runway crossings (e.g. CYQB pistes 06/24 × 11/29).
--                           (2) Orphaned gate detection — after loading, each gate is
--                           BFS-tested for taxiway reachability. Orphaned gates are
--                           excluded from the menu and a spoken message warns the pilot.
--                           (3) Spoken A* failure messages — when no route is found
--                           between a terminal and a runway, a clear spoken message
--                           tells the pilot why.
--    ---------------------------------------------------------------------------------
--    ---------------------------------------------------------------------------------
...
local t_virtual_intersections_added = 0  -- VER1.13 : count of virtual nodes created
...
function apply_simbrief_runway()
    ...
    -- Vérifier si la piste existe dans l'aéroport
    for i = 1, #t_runway do
    ...
    -- Si la piste n'existe pas → ne rien bloquer
end
...
function chk_line_of_sight(shooter_heading, shooter_left_angle, shooter_right_angle, shooter_x, shooter_z, target_x, target_z )
  ...
  if l_heading_to_target >= l_left_arc and l_heading_to_target <= l_left_arc + shooter_left_angle + shooter_right_angle  then
      l_within_sight = true
  elseif l_heading_to_target <= l_right_arc and l_heading_to_target >= l_right_arc - ( shooter_left_angle + shooter_right_angle ) then
      l_within_sight = true 
  end
    ...
    elseif in_dist >= avg_dist_from_plane and in_dist < max_dist_from_plane and in_car_in_sight then
        if car_speed > 0 then
          if l_ref_spd > car_speed then     
              ...
              if car_accel > accel_max then car_accel = accel_max end
...
function plot_position( in_act_dist )
  ...
  local l_rear_wheel_heading = 0
    ...
    if curr_node + 1 ~= #t_node then
        remaining_dist_leg = 0
        l_rear_wheel_x, l_rear_wheel_z = coordinates_of_adjusted_ref(car_x, car_z, 0, car_rear_wheel_to_ref * -1, car_body_heading )
        l_head1, remaining_dist_leg = heading_n_dist( l_rear_wheel_x, l_rear_wheel_z, t_node[curr_node+1].x, t_node[curr_node+1].z)
    else
    ...
    if l_remaining_leg_dist <= 0 then
        ...
        if t_node[curr_node+1].dir ~= 0  and curr_node + 1 ~= #t_node then
            turning_is_active = 1
            t_node[curr_node+1].rot_x, t_node[curr_node+1].rot_z = CoR_coordinates_using_car_ref (car_x, car_z, l_head1, t_node[curr_node+1].radius, t_node[curr_node+1].dir)
        else
  ...
  if turning_is_active == 1 then     
      ...
      if t_node[curr_node+1].head1_exit == nil then  
          ...
          l_head2, l_dist = heading_n_dist( t_node[curr_node+1].rot_x, t_node[curr_node+1].rot_z, t_node[curr_node+2].x, t_node[curr_node+2].z)
      ...
      l_remaining_rot = compute_angle_diff_dir ( car_body_heading , t_node[curr_node+1].head1_exit, t_node[curr_node+1].dir )
      ...
      l_heading_from_center = minus_delta_clockwise(car_body_heading, 90 - t_node[curr_node+1].angle_rear_to_ref, t_node[curr_node+1].dir)
      car_x = t_node[curr_node+1].rot_x + math.sin ( math.rad ( l_heading_from_center ) ) * t_node[curr_node+1].ref_rot_radius
      car_z = t_node[curr_node+1].rot_z + math.cos ( math.rad (  l_heading_from_center ) ) * t_node[curr_node+1].ref_rot_radius * -1         
      ...
      if turning_is_active == 2 then      
          ...
          t_node[curr_node+1].rot2_x, t_node[curr_node+1].rot2_z = CoR_coordinates_using_car_ref (car_x, car_z, car_body_heading, t_node[curr_node+1].radius, t_node[curr_node+1].dir * -1 ) 
          l_head2, l_dist = heading_n_dist( t_node[curr_node+1].rot2_x, t_node[curr_node+1].rot2_z, t_node[curr_node+2].x, t_node[curr_node+2].z)
          l_AoR = math.deg ( math.acos( t_node[curr_node+1].radius / l_dist ) )  
          l_heading_from_center = minus_delta_clockwise(l_head2, l_AoR, t_node[curr_node+1].dir * -1 ) 
          t_node[curr_node+1].head1_exit = add_delta_clockwise(l_heading_from_center, 90, t_node[curr_node+1].dir * -1 )     
      end
  ...
  if turning_is_active == 2 then     
      ...
      l_remaining_rot = compute_angle_diff_dir ( car_body_heading , t_node[curr_node+1].head1_exit, t_node[curr_node+1].dir * -1)
      l_remaining_turn_dist = ( l_remaining_rot / 360 ) * ( 2 * math.pi *  t_node[curr_node+1].radius )
      l_remaining_act_dist = l_act_dist - l_remaining_turn_dist
      ...
      l_heading_from_center = minus_delta_clockwise(car_body_heading, 90 - t_node[curr_node+1].angle_rear_to_ref, t_node[curr_node+1].dir * -1)
      car_x = t_node[curr_node+1].rot2_x + math.sin ( math.rad ( l_heading_from_center ) ) * t_node[curr_node+1].ref_rot_radius
      car_z = t_node[curr_node+1].rot2_z + math.cos ( math.rad (  l_heading_from_center ) ) * t_node[curr_node+1].ref_rot_radius * -1         
  ...
  if car_sign == 0 or curr_node >= #t_node - 2 then 
      ...
      if curr_node ~= #t_node then
          if remaining_dist_leg < avg_dist_from_plane and t_node[curr_node+1].dir ~= nil and t_node[curr_node+1].AoC < 160 then
          ...
          -- VER1.8 : for back-taxi, skip the hotzone stop entirely
          -- The else block (curr_node == #t_node) handles the final stop at the threshold
          ...
          if depart_arrive ~= 1 then
              local l_gate_in_sight, l_to_gate_heading, _ = chk_line_of_sight ( car_body_heading, 45, 45, 
                                                                                car_x, car_z, 
...
function determine_steering (in_AoR, in_remaining_turn_dist, in_dir, in_steer_limit)
    ...
    if time_to_exit_turn < time_to_steer_neutral  then
        if in_dir  == 1 then
            ...
            if steering <= 0 then steering = 0 end  
        else
            steering = steering + l_steering_delta
            if steering >= 0 then steering = 0 end           
        end            
    elseif l_prev_steer_angle < in_steer_limit and ( time_to_exit_turn > time_to_steer_neutral ) then
        if in_dir  == 1 then
            steering = steering + l_steering_delta
            if steering >= in_steer_limit then steering = in_steer_limit end  
        else
            steering = steering - in_AoR
            if steering <= in_steer_limit * -1 then steering = in_steer_limit * -1 end           
        end
    end
...
function minus_delta_clockwise(in_heading, in_delta, in_direction)
  ...
  if in_direction == 1 then
     ...
     if l_heading < 0 then l_heading = l_heading + 360 end
     return l_heading
...
function get_local_coordinates(in_lat, in_lon, in_alt)
  ...
  if l_alt == 0 then 
    ...
    in_lat, in_lon, in_alt = local_to_latlon(probeinfo_value[0].locationX, probeinfo_value[0].locationY, probeinfo_value[0].locationZ)
  end
...
function load_object()
  ...
  if car_type_fmcar == "Ferrari" or (car_type_fmcar == "Auto" and l_auto_sel == 1) then
      XPLM.XPLMLoadObjectAsync( syspath .. "Resources/default scenery/airport scenery/Dynamic_Vehicles/crew_car_ferrari.obj",
                function(inObject, inRefcon) 
                obj_instance[0] = XPLM.XPLMCreateInstance(inObject, datarefs_addr)
                objref = inObject 
                end,
                inRefcon )
      car_weight = 1445; tire_diameter = 0.79; min_turn_radius = 10.8
      width_btw_midtire = 1.86; car_front_to_back_wheel = 3.2; car_rear_wheel_to_ref = 1.60
  ...
  elseif car_type_fmcar == "Van" or (car_type_fmcar == "Auto" and l_auto_sel == 2) then
      XPLM.XPLMLoadObjectAsync( SCRIPT_DIRECTORY .. "follow_me/objects/fm_van.obj",
                ...
                inRefcon ) 
      car_weight = 1700; tire_diameter = 0.65; min_turn_radius = 10.8
      width_btw_midtire = 1.39; car_front_to_back_wheel = 2.64; car_rear_wheel_to_ref = 1.32
  ...
  elseif car_type_fmcar == "Truck" or (car_type_fmcar == "Auto" and l_auto_sel == 3) then
      XPLM.XPLMLoadObjectAsync( SCRIPT_DIRECTORY .. "follow_me/objects/fm_truck.obj",
                ...
                inRefcon ) 
      car_weight = 2000; tire_diameter = 0.730; min_turn_radius = 10.8
      width_btw_midtire = 1.578; car_front_to_back_wheel = 3.137; car_rear_wheel_to_ref = 1.5685
      time_to_100 = 7.5; speed_max = 54; place_above_the_car = 1.86; place_Z_of_car = -0.4347
  end 
...
function unload_object()
    if obj_instance[0] ~= nil then XPLM.XPLMDestroyInstance(obj_instance[0]) end
    if objref ~= nil then XPLM.XPLMUnloadObject(objref) end
    obj_instance[0] = nil; objref = nil
    if signboard_instance[0] ~= nil then XPLM.XPLMDestroyInstance(signboard_instance[0]) end
    if signboardref ~= nil then XPLM.XPLMUnloadObject(signboardref) end
    signboard_instance[0] = nil; signboardref = nil
end
...
function unload_path()
    ...
    if pathref ~= nil then XPLM.XPLMUnloadObject(pathref) end    
    path_instance[0] = nil; pathref = nil; path_is_shown = false         
end
...
function unload_rampstart()
    ...
    if rampstartref ~= nil then XPLM.XPLMUnloadObject(rampstartref) end
    rampstart_instance[0] = nil; rampstartref = nil; rampstart_chg = false
end

function register_dataref()

    -- VER1.12 : FlyWithLua restarts the Lua engine on airport/plane change (LUA_RUN increments).
    -- register_dataref() is called each time. If datarefs are already registered,
    -- XPLMRegisterDataAccessor returns a duplicate pointer → access violation → crash.
    -- Fix : always unregister first if already registered.
    if dataref_register1 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register1); dataref_register1 = nil end
    if dataref_register7 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register7); dataref_register7 = nil end
    if dataref_register2 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register2); dataref_register2 = nil end
    if dataref_register3 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register3); dataref_register3 = nil end
    if dataref_register4 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register4); dataref_register4 = nil end
    if dataref_register5 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register5); dataref_register5 = nil end
    if dataref_register6 ~= nil then XPLM.XPLMUnregisterDataAccessor(dataref_register6); dataref_register6 = nil end
              ...
              while true do
                  ...
                  if l_airport_found == false then 
                      ...
                      while true do
                          ...
                          if l_airport_found == true then break end    
                      end
                  end
                  ...
                  if l_airport_found then 
                      for l_line2 in l_new_lines:gmatch("[^\r\n]+") do
                          if l_not_first_line == false then 
                              ...
                              if string.find( l_line2,  "^1%s") or l_line2 == "99" then   
                                  ...
                                  build_virtual_intersections()  -- VER1.13 : add missing taxiway/runway intersection nodes
                          ...
                          if string.match( l_line2,  "^100%s" ) then decipher_runway(l_line2, t_runway) end  
                          if string.match( l_line2,  "^1301%s" ) and l_processed_1300 == true then decipher_ramp_operation(l_line2) end                    
                          if string.match( l_line2,  "^1300%s" ) then l_processed_1300 = decipher_ramp(l_line2, t_gate) end  
                      ...
                      if l_terminate_loop == true then break end    
                  end
              end
              l_file2:close()
              if l_terminate_loop == true then break end  
...
function decipher_runway(in_str)
  ...
  t_runway[i] = {}
  t_runway[i+1] = {}    
  t_runway[i].ID, l_str1, l_str2, 
  t_runway[i+1].ID, l_str3, l_str4 = 
          string.match( in_str,   "100 %s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*[^%s]+%s*" ..
                                  "([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*" .. 
...
function decipher_ramp(in_str)
  ...
  l_str1, l_str2, l_str3, l_str4, l_str5, l_str6 = string.match( in_str,  "1300 %s*([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*(.*)")  
  i = #t_gate + 1
  t_gate[i] = {}
  ...
  elseif string.find (t_gate[i].Types, "all") then 
    ...
    if string.find (t_gate[i].Types, "heavy") then l_heavy = "1 2 " end  
    if string.find (t_gate[i].Types, "jets") then   
      if l_heavy ~= "" then l_jet = "3 5 " else l_jet = "3 5 7 " end  
    end  
    if string.find (t_gate[i].Types, "turboprops") then  
      if l_jet == "3 5 " then l_jet = "3 5 7 " end   
      l_turbo = "4 6 "
    end    
...
function decipher_taxinode(in_str)
  ...
  t_taxinode[i].x, t_taxinode[i].y, t_taxinode[i].z, world_alt = get_local_coordinates(t_taxinode[i].Lat, t_taxinode[i].Lon, world_alt) 
end
...
function decipher_taxisegment(in_str)
  ...
  i = #t_segment + 1
  t_segment[i] = {}
  l_str1, l_str2, l_str3, l_str4, l_str5 = string.match(in_str, "1202 %s*([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*([^%s]+)%s*")
  ...
  t_segment[i].Hotzone = ""
  t_segment[i].Heading, t_segment[i].Dist = heading_n_dist( t_taxinode[t_segment[i].Node1+1].x, 
...
function decipher_taxisegment_hotzone(in_str)
  ...
  if t_segment[i].Type == "taxiway" then
    if t_taxinode[t_segment[i].Node1 + 1].Type ~= "runway" then t_taxinode[t_segment[i].Node1 + 1].Type = "hotzone" end
    if t_taxinode[t_segment[i].Node2 + 1].Type ~= "runway" then t_taxinode[t_segment[i].Node2 + 1].Type = "hotzone" end    
  end  
end

function build_virtual_intersections()
  -- VER1.14 : SPIDER WEB — split every segment at every geometric intersection.
  -- Works in repeated passes until no new intersections are found.
  -- Lua 5.1 compatible (no goto, no ::label::).

  -- ── Helper : find node within radius ──────────────────────────────────────
  local function find_near(l_x, l_z, l_thr)
      for l_ni = 1, #t_taxinode do
          if t_taxinode[l_ni] and t_taxinode[l_ni].x then
              local l_dd = math.sqrt((l_x - t_taxinode[l_ni].x)^2 +
                                     (l_z - t_taxinode[l_ni].z)^2)
              if l_dd < l_thr then return l_ni - 1 end
          end
      end
      return -1
  end

  -- ── Helper : create virtual node ──────────────────────────────────────────
  local function vnode_new(l_vx, l_vy, l_vz, l_typ, l_nm)
      local l_vid = #t_taxinode
      t_taxinode[l_vid + 1] = {
          x = l_vx, y = l_vy, z = l_vz,
          Lat = 0, Lon = 0,
          Type = l_typ, Runway = "", Segment = "",
          f_value = nil, g_value = nil, h_value = nil,
          parent = nil, cost = nil, heading = nil
      }
      if local_to_latlon then
          t_taxinode[l_vid+1].Lat, t_taxinode[l_vid+1].Lon, _ =
              local_to_latlon(l_vx, l_vy, l_vz)
      end
      return l_vid
  end

  -- ── Helper : register segment index in a node's Segment string ────────────
  local function seg_reg(l_nid, l_sidx)
      if not t_taxinode[l_nid + 1] then return end
      local l_seg = t_taxinode[l_nid + 1].Segment or ""
      if l_seg == "" then
          t_taxinode[l_nid + 1].Segment = tostring(l_sidx)
      else
          t_taxinode[l_nid + 1].Segment = l_seg .. "," .. tostring(l_sidx)
      end
  end

  -- ── Helper : add new segment ───────────────────────────────────────────────
  local function seg_add(l_na, l_nb, l_typ, l_nm)
      local l_s = #t_segment + 1
      t_segment[l_s] = {
          Node1 = l_na, Node2 = l_nb, Dir = "twoway",
          Type = l_typ, ID = l_nm, Hotzone = "", Size = ""
      }
      local l_hd, l_di = heading_n_dist(
          t_taxinode[l_na+1].x, t_taxinode[l_na+1].z,
          t_taxinode[l_nb+1].x, t_taxinode[l_nb+1].z)
      t_segment[l_s].Heading = l_hd
      t_segment[l_s].Dist    = l_di
      seg_reg(l_na, l_s)
      seg_reg(l_nb, l_s)
  end

  -- ── Helper : remove segment registration from its nodes ───────────────────
  local function seg_unregister(l_sidx)
      local l_s = t_segment[l_sidx]
      if not l_s then return end
      for _, l_nid in ipairs({l_s.Node1, l_s.Node2}) do
          if t_taxinode[l_nid + 1] then
              local l_parts = {}
              for l_tok in (t_taxinode[l_nid+1].Segment or ""):gmatch("[^,]+") do
                  if tonumber(l_tok) ~= l_sidx then
                      l_parts[#l_parts + 1] = l_tok
                  end
              end
              t_taxinode[l_nid + 1].Segment = table.concat(l_parts, ",")
          end
      end
      t_segment[l_sidx] = nil
  end

  -- ── Helper : geometric segment intersection ────────────────────────────────
  local function seg_intersect(l_ax,l_az, l_bx,l_bz, l_cx,l_cz, l_dx,l_dz)
      local l_dxAB, l_dzAB = l_bx - l_ax, l_bz - l_az
      local l_dxCD, l_dzCD = l_dx - l_cx, l_dz - l_cz
      local l_den = l_dxAB * l_dzCD - l_dzAB * l_dxCD
      if math.abs(l_den) < 1e-8 then return nil end
      local l_t = ((l_cx - l_ax) * l_dzCD - (l_cz - l_az) * l_dxCD) / l_den
      local l_u = ((l_cx - l_ax) * l_dzAB - (l_cz - l_az) * l_dxAB) / l_den
      if l_t > 0.001 and l_t < 0.999 and l_u > 0.001 and l_u < 0.999 then
          return l_ax + l_t * l_dxAB, l_az + l_t * l_dzAB
      end
      return nil
  end

  -- ════════════════════════════════════════════════════════════════════════════
  -- PASS 1 : Spider web — split at every intersection, repeat until stable
  -- ════════════════════════════════════════════════════════════════════════════
  local l_total_splits = 0

  for l_pass = 1, 10 do
      -- Snapshot of valid segment indices
      local l_active = {}
      for l_si = 1, #t_segment do
          if t_segment[l_si] then
              l_active[#l_active + 1] = l_si
          end
      end
      ...
      for l_ii = 1, #l_active do
          local l_si = l_active[l_ii]
          if not l_done[l_si] and t_segment[l_si] then
              local l_n1i = t_segment[l_si].Node1
              local l_n2i = t_segment[l_si].Node2
              local l_tyi = t_segment[l_si].Type
              local l_nmi = t_segment[l_si].ID
              if t_taxinode[l_n1i+1] and t_taxinode[l_n2i+1] then
                  local l_ax = t_taxinode[l_n1i+1].x
                  local l_az = t_taxinode[l_n1i+1].z
                  local l_bx = t_taxinode[l_n2i+1].x
                  local l_bz = t_taxinode[l_n2i+1].z

                  local l_found_j = false
                  local l_jj = l_ii + 1
                  while l_jj <= #l_active and not l_found_j do
                      local l_sj = l_active[l_jj]
                      if not l_done[l_sj] and t_segment[l_sj] then
                          local l_n1j = t_segment[l_sj].Node1
                          local l_n2j = t_segment[l_sj].Node2
                          local l_tyj = t_segment[l_sj].Type
                          local l_nmj = t_segment[l_sj].ID
                          if t_taxinode[l_n1j+1] and t_taxinode[l_n2j+1] then
                              local l_cx = t_taxinode[l_n1j+1].x
                              local l_cz = t_taxinode[l_n1j+1].z
                              local l_dx = t_taxinode[l_n2j+1].x
                              local l_dz = t_taxinode[l_n2j+1].z
                              local l_ix, l_iz = seg_intersect(
                                  l_ax,l_az, l_bx,l_bz, l_cx,l_cz, l_dx,l_dz)
                              if l_ix then
                                  local l_vid = find_near(l_ix, l_iz, 3.0)
                                  if l_vid < 0 then
                                      local l_vy = (t_taxinode[l_n1i+1].y +
                                                    t_taxinode[l_n1j+1].y) / 2
                                      local l_vtyp = (l_tyi == "runway" or
                                                      l_tyj == "runway") and
                                                     "runway" or "taxiway"
                                      l_vid = vnode_new(l_ix, l_vy, l_iz,
                                                        l_vtyp, l_nmi.."x"..l_nmj)
                                      l_splits = l_splits + 1
                                  end
                                  seg_unregister(l_si)
                                  seg_unregister(l_sj)
                                  seg_add(l_n1i, l_vid, l_tyi, l_nmi)
                                  seg_add(l_vid,  l_n2i, l_tyi, l_nmi)
                                  seg_add(l_n1j, l_vid, l_tyj, l_nmj)
                                  seg_add(l_vid,  l_n2j, l_tyj, l_nmj)
                                  if l_tyi == "runway" or l_tyj == "runway" then
                                      t_taxinode[l_vid+1].Type = "runway"
                                  end
                                  l_done[l_si] = true
                                  l_done[l_sj] = true
                                  l_found_j    = true
                                  logMsg("FollowMe VER1.14 : split " ..
                                         l_nmi .. "x" .. l_nmj ..
                                         " p=" .. l_pass ..
                                         " n=" .. l_vid)
                              end
      ...
      l_total_splits = l_total_splits + l_splits
      if l_splits == 0 then
          logMsg("FollowMe VER1.14 : spider web done " ..
                 l_pass .. " pass(es) " ..
                 l_total_splits .. " node(s) " .. curr_ICAO)
          break
      end
  end

  -- ════════════════════════════════════════════════════════════════════════════
  -- PASS 2 : Orphaned gate detection
  -- ════════════════════════════════════════════════════════════════════════════
  local l_twy_set = {}
  for l_si = 1, #t_segment do
      if t_segment[l_si] and t_segment[l_si].Type ~= "runway" then
          l_twy_set[t_segment[l_si].Node1] = true
          l_twy_set[t_segment[l_si].Node2] = true
      end
  end

  local function bfs_twy(l_start)
      local l_vis = {[l_start] = true}
      local l_q   = {l_start}
      local l_h   = 1
      while l_h <= #l_q do
          local l_cur = l_q[l_h]; l_h = l_h + 1
          if l_twy_set[l_cur] then return true end
          if t_taxinode[l_cur+1] and t_taxinode[l_cur+1].Segment ~= "" then
              for l_tok in t_taxinode[l_cur+1].Segment:gmatch("[^,]+") do
                  local l_si2 = tonumber(l_tok)
                  if l_si2 and t_segment[l_si2] then
                      local l_nb = (t_segment[l_si2].Node1 == l_cur)
                                   and t_segment[l_si2].Node2
                                   or  t_segment[l_si2].Node1
                      if not l_vis[l_nb] then
                          l_vis[l_nb] = true
                          l_q[#l_q + 1] = l_nb
                      end
                  end
          ...
          if l_h > 400 then break end
      end
      return false
  end

  local l_spoken = false
  for l_gi = 1, #t_gate do
      local l_best_d = 99999
      local l_best_n = -1
      for l_ni = 1, #t_taxinode do
          if t_taxinode[l_ni] and t_taxinode[l_ni].x then
              local l_ddx = t_taxinode[l_ni].x - t_gate[l_gi].x
              local l_ddz = t_taxinode[l_ni].z - t_gate[l_gi].z
              local l_dd  = math.sqrt(l_ddx * l_ddx + l_ddz * l_ddz)
              if l_dd < l_best_d then l_best_d = l_dd; l_best_n = l_ni - 1 end
          end
      end
      if l_best_n >= 0 and not bfs_twy(l_best_n) then
          t_gate[l_gi].orphan = true
          logMsg("FollowMe VER1.14 : gate " .. (t_gate[l_gi].ID or "?") ..
                 " orphaned")
          if not l_spoken then
              XPLMSpeakString("No taxiway connection for this terminal")
              l_spoken = true
          end
      else
          t_gate[l_gi].orphan = false
      end
  end
...
function match_runway (in_runway_idx)
  -- VER1.11 : Search ALL runway nodes (including duplicates).
  -- Prefer nodes that have at least one taxiway neighbour (reachable from taxi net).
  -- Among those, pick the closest to the runway threshold.
  -- Fallback: if none has a taxiway neighbour, pick the plain closest.
  local l_idx = 0
  local l_curr_dist = 0
  local l_min_dist_twy  = 99999   -- best dist among nodes WITH taxiway neighbour
  local l_min_dist_any  = 99999   -- best dist among ALL runway nodes (fallback)
  local l_best_twy      = -1      -- best node with taxiway neighbour
  local l_best_any      = -1      -- best node overall
  ...
  for l_idx = 1, #t_taxinode do
      if t_taxinode[l_idx].Type == "runway" then
          local l_node_id = l_idx - 1  -- taxinode is 1-based, node_id is 0-based
          ...
          -- Check if this node has a taxiway neighbour
  ...
  -- Pick best node with taxiway neighbour; fallback to any closest
  local l_chosen = l_best_twy
  local l_chosen_dist = l_min_dist_twy
  if l_chosen == -1 then
      l_chosen = l_best_any
      l_chosen_dist = l_min_dist_any
      logMsg("FollowMe VER1.11 : RWY " .. t_runway[in_runway_idx].ID ..
  ...
  if l_chosen >= 0 then
      ...
      t_taxinode[l_chosen+1].Runway = t_runway[in_runway_idx].ID
  end
end
...
function handle_plugin_window()
    ...
    if FM_car_active == false and get_from_SimBrief == true 
                and fm_gear1_gnd == 1 and holder_wnd then
        if ( prev_taxi_light ~= fm_taxi_light and fm_taxi_light == 1 ) or 
                ( prev_beacon_light ~= fm_beacon_light and fm_beacon_light == 1 ) then
    ...
    if speed_limiter and FM_car_active and
       ...
       fm_gnd_spd > 10.288 and fm_run_time > speed_warn_time then
        play_sound(snd_keep_speed)
        speed_warn_time = fm_run_time + 15   -- repeat at most every 15 seconds
    end
  
    if FM_car_active == false and flightstart == 9999 and
          ( fm_gear1_gnd == 1 and fm_gear2_gnd == 1 ) and ground_time == 0 and 
              fm_taxi_light == 1 and fm_gnd_spd < 15 then
  ...
  if holder_wnd then
    if MOUSE_X >= SCREEN_WIDTH - Holder_len and MOUSE_X <= SCREEN_WIDTH and 
       MOUSE_Y >= SCREEN_HIGHT - ( Win_Y + Holder_len ) and MOUSE_Y <= SCREEN_HIGHT - Win_Y then 
  ...
  if followme_wnd ~= nil then
    ...
    local l_over_holder = (MOUSE_X >= SCREEN_WIDTH - Holder_len and MOUSE_X <= SCREEN_WIDTH and 
                           MOUSE_Y >= SCREEN_HIGHT - ( Win_Y + Holder_len ) and MOUSE_Y <= SCREEN_HIGHT - Win_Y)
    if not l_over_holder then
      if MOUSE_X >= SCREEN_WIDTH - 400 - Holder_len and MOUSE_X <= SCREEN_WIDTH - Holder_len and 
         MOUSE_Y >= SCREEN_HIGHT - ( Win_Y + 400 ) and MOUSE_Y <= SCREEN_HIGHT - Win_Y then 
...
function build_window(wnd, x, y)
  ...
  local l_flags = bit.bor (imgui.constant.WindowFlags.NoTitleBar, 
  ...
  if FM_car_active == true or #t_runway == 0 then
    ...
    if FM_car_active == false then imgui.SetNextWindowFocus() end
    ...
    if imgui.Begin("##Disabled", nil,l_flags) then imgui.End() end
    imgui.PopStyleVar()
    imgui.PopStyleVar()
    if FM_car_active == false then imgui.SetNextWindowFocus() end
  end
  ...
  if sb_fetch_status == "OK" and sb_origin_icao ~= "" then
  ...
  if sb_fetch_status == "OK" and sb_dest_icao ~= "" then
  ...
  if get_from_SimBrief then   
    imgui.InputText("##text_runway", depart_runway, 5, imgui.constant.InputTextFlags.ReadOnly)
  else    
    ...
    if imgui.BeginCombo("##select_runway", l_depart_runway) then      
      for i = 1, #t_runway  do
          ...
          if l_is_selected then imgui.SetItemDefaultFocus() end
      end
      imgui.EndCombo()
    end 
  end
  ...
  if combo_filter_list == true then
    ...
    if imgui.Begin("##combo_filter", nil, l_flags) then
      local l_gate = ""
      if arrival_gate > 0 then l_gate = t_gate[arrival_gate].ID end
      t_suitable_gates = {}
      for i = 1, #t_gate  do
          -- VER1.14 : skip orphaned gates (no taxiway connection)
          if not t_gate[i].orphan then
  ...
  if  l_err == "-17" then 
    ...
    if imgui.Button("X##clear_filter", 15, 15) then
        arrival_gate = 0; gatetext = ""; combo_filter_list = false
        text_was_chg = false; rampstart_chg = true        
    end
  ...
  if FM_car_active == true and depart_arrive == 2 then
    ...
    if imgui.BeginCombo("##select_type", l_ac_types) then
      for i = 1, #ac_types  do
          ...
          if imgui.Selectable(ac_types[i], l_is_selected) then
              ...
              gatetext = ""; arrival_gate = 0; rampstart_chg = true     
              Err_Msg[1] = {}; Err_Msg[2] = {}; Err_Msg[3] = {}       
              depart_gate = check_gate()
          end
          if l_is_selected then imgui.SetItemDefaultFocus() end
    ...
    if imgui.Button("X##clear_type", 20, 20) then
      Aircraft_Type = ""; gatetext = ""; arrival_gate = 0; rampstart_chg = true
      Err_Msg[1] = {}; Err_Msg[2] = {}; Err_Msg[3] = {}  
      depart_gate = check_gate()
    end
  end
  ...
  if l_changed then
    show_path = l_newval
    if show_path then load_path() else unload_path() end  
  end
  ...
  -- Vehicle selection
  imgui.SetCursorPosX(20); imgui.SetCursorPosY(413)
  if imgui.RadioButton(" Ferrari", car_type_fmcar == "Ferrari", false) then car_type_fmcar = "Ferrari" end
  imgui.SetCursorPosX(110); imgui.SetCursorPosY(413)
  if imgui.RadioButton(" FM Van", car_type_fmcar == "Van", false) then car_type_fmcar = "Van" end
  imgui.SetCursorPosX(195); imgui.SetCursorPosY(413)
  if imgui.RadioButton(" FM Truck", car_type_fmcar == "Truck", false) then car_type_fmcar = "Truck" end
  imgui.SetCursorPosX(290); imgui.SetCursorPosY(413)
  if imgui.RadioButton(" Auto Select", car_type_fmcar == "Auto", false) then car_type_fmcar = "Auto" end
  ...
  l_changed, l_newtext = imgui.InputText("##simbrief_id", simbrief_id, 11,
                            imgui.constant.InputTextFlags.CharsDecimal)
  if l_changed then
  ...
  imgui.SetCursorPosX(10)
  if sb_fetch_status == "OK" then
      if get_from_SimBrief and sb_airport_mismatch then
          imgui.PushStyleColor(imgui.constant.Col.Text, 0xFF0000FF)
          imgui.TextUnformatted("No SimBrief data for the current airport")
          imgui.PopStyleColor()
      else
  ...
  for i= 1, 3 do
    ...
    imgui.SetCursorPosX(20)
    if not Err_Msg[i].text then break end
    if i == 1 then 
...
function update_msg(in_msg)
    ...
    if in_msg == "-18" then
        ...
        if Err_Msg[1].text ~= nil and Err_Msg[1].text == in_msg then return end  
    end    
...
function determine_XP_route()
    ...
    if depart_arrive == 0 then return "-5" end   -- no mode selected yet
...
function determine_possible_routes()
    ...
    if depart_gate ~= 0 then 
        l_found, l_startpt_node, l_startpt_x, l_startpt_z = determine_pos_on_segment ( t_gate[depart_gate].Heading, 
        ...
        l_found, l_startpt_node, l_startpt_x, l_startpt_z = determine_pos_on_segment ( fm_plane_head, 
                                                                                       l_adj_fm_plane_x, 
                                                                                       l_adj_fm_plane_z, 
                                                                                       "tie_down" ) 
        if l_found == false then
            l_found, l_startpt_node, l_startpt_x, l_startpt_z = determine_pos_on_segment ( l_rev_heading, fm_plane_x, fm_plane_z, "tie_down")
        end
        if l_found == false and depart_arrive == 2 then return "-16" end
    end
    
    if l_found == false then return "-12" end
    ...
    if depart_arrive == 1 then
        ...
        -- VER1.11 : simplified is_backtaxi detection
        -- Since match_runway() now always picks a taxiway-connected node,
        -- is_backtaxi simply means the entry node is far from the threshold (>50m).
        -- In that case a virtual GPS node at the exact threshold is needed.
        is_backtaxi = false
        local _, l_entry_to_threshold = heading_n_dist(
            t_taxinode[l_endpt_node+1].x, t_taxinode[l_endpt_node+1].z,
            t_runway[1].x, t_runway[1].z)  -- temporary, overwritten below
        -- Find the actual runway threshold distance
        for l_index = 1, #t_runway do
            if t_runway[l_index].ID == depart_runway then
                _, l_entry_to_threshold = heading_n_dist(
                    t_taxinode[l_endpt_node+1].x, t_taxinode[l_endpt_node+1].z,
                    t_runway[l_index].x, t_runway[l_index].z)
                break
            end
        end
        if l_entry_to_threshold > 200 then  -- VER1.12 fix: 200m avoids false back-taxi on normal runways (e.g. CYHU 24R at 90m)
            is_backtaxi = true
            logMsg("FollowMe VER1.11 : RWY " .. depart_runway ..
                   " entry node is " .. string.format("%.1f", l_entry_to_threshold) ..
                   "m from threshold — virtual GPS node will be added")
        end
    else
        l_found, l_endpt_node, l_endpt_x, l_endpt_z = determine_pos_on_segment( t_gate[arrival_gate].Heading, 
                                                                                ...
                                                                                "gate")     
        if l_found == false then return "-13" end    
    end  
     
    impose_restriction_chk = true
    
    for l_index = 1, #t_taxinode do
        _, t_taxinode[l_index].h_value = heading_n_dist( t_taxinode[l_index].x, t_taxinode[l_index].z, 
                                                         t_taxinode[l_endpt_node+1].x, t_taxinode[l_endpt_node+1].z )
    end    
    ...
    if #t_possible_route == 0 then 
        ...
        transverse(l_startpt_node, l_endpt_node, -1) 
        if #t_possible_route == 0 then
            -- VER1.14 : speak a meaningful message depending on context
            if depart_arrive == 1 and depart_gate > 0 then
                XPLMSpeakString("No taxiway route to the runway from this terminal")
            elseif depart_arrive == 2 and arrival_gate > 0 then
                XPLMSpeakString("No taxiway route from the runway to this terminal")
            end
            return "-14"
        end
    end    
...
function transverse(in_startnode, in_endnode, in_heading)
    ...
    function evaluate_node(in_node, in_size, t_open, t_close)
        ...
        for i = 1, #t_open do
            if t_open[i].Node == in_node then l_in_open = true; break end
        end
        for i = 1, #t_close do
            if t_close[i].Node == in_node then l_in_close = true; break end
        end
        if l_in_open == true or l_in_close == true then return false end
        if not impose_restriction_chk then return true
        else
            if in_size ~= "" then
                local l_aircraft_type = tonumber(Aircraft_Type)
                if in_size == "A"  then
                    if ( l_aircraft_type > 0 and l_aircraft_type < 7 ) then return false end    
                elseif ( in_size == "B" or in_size == "C" or in_size == "D" ) then
                    if ( l_aircraft_type > 0 and l_aircraft_type < 3 ) then return false end    
                elseif in_size == "E"  then
                    if ( l_aircraft_type == 1 ) then return false end                   
                end               
            end    
            return true
        end         
    ...
    while l_curr_node ~= in_endnode and l_curr_node ~= -1 do
        for l_string in t_taxinode[l_curr_node+1].Segment:gmatch("[^,]+") do
            ...
            l_pass = false    
            if t_segment[l_segment_idx].Type ~= "runway" or ( t_segment[l_segment_idx].Type == "runway" and not impose_restriction_chk ) then 
            ...
            if l_pass then
                ...
                if t_taxinode[l_node+1].f_value == nil  or l_f_value < t_taxinode[l_node+1].f_value then
                    ...
                    if t_taxinode[l_curr_node+1].heading == nil then 
                        ...
                        if t_taxinode[l_curr_node+1].cost == nil then t_taxinode[l_curr_node+1].cost = 0 end
                        l_AoC = 180 - compute_angle_diff (t_taxinode[l_curr_node+1].heading, l_curr_heading )
                        if l_AoC < 20 then t_taxinode[l_node+1].cost = t_taxinode[l_curr_node+1].cost + 150
                        elseif l_AoC >= 20 and l_AoC <=80 then t_taxinode[l_node+1].cost = t_taxinode[l_curr_node+1].cost + 12 
                        elseif l_AoC > 80 and l_AoC < 90 then t_taxinode[l_node+1].cost = t_taxinode[l_curr_node+1].cost + 3 
                        elseif l_AoC >= 90 and l_AoC <= 140 then t_taxinode[l_node+1].cost = t_taxinode[l_curr_node+1].cost + 1
                        elseif l_AoC > 140 and l_AoC <= 170 then t_taxinode[l_node+1].cost = t_taxinode[l_curr_node+1].cost + 1  
                        elseif l_AoC > 170 then t_taxinode[l_node+1].cost = t_taxinode[l_curr_node+1].cost  end  
                    end
        ...
        table.sort( t_open, function(a,b) return a.order > b.order or (a.order == b.order and a.cost > b.cost) end)
        if #t_open > 0 then    
...
function process_possible_routes()
    ...
    for l_index = 1, #t_node - 2 do
        if t_node[l_index].dist < l_min_dist_btw_nodes then
            t_node[l_index+1].x, t_node[l_index+1].z = coordinates_of_adjusted_ref(t_node[l_index+1].x, t_node[l_index+1].z, 
                                                                                       0, ( l_min_dist_btw_nodes - t_node[l_index].dist ), 
                                                                                       t_node[l_index].heading )
            t_node[l_index].dist = l_min_dist_btw_nodes 
            t_node[l_index+1].heading, t_node[l_index+1].dist = heading_n_dist( t_node[l_index+1].x, t_node[l_index+1].z, 
                                                                                t_node[l_index+2].x, t_node[l_index+2].z )                                                      
        end   
    end   
  
    while t_taxinode[#t_taxinode].Type == "New" do t_taxinode[#t_taxinode] = nil end
    
    while t_segment[#t_segment].ID == "ADD_NEWSEGMENT" do
        if t_taxinode[t_segment[#t_segment].Node1 + 1] ~= nil then
            t_taxinode[t_segment[#t_segment].Node1 + 1].Segment = string.gsub(t_taxinode[t_segment[#t_segment].Node1 + 1].Segment, ","..tostring(#t_segment), "")
        end
    ...
    -- VER1.9 : back-taxi GPS threshold node
    -- When the runway has no taxiway exit (back-taxi), add a virtual node at the
    -- exact GPS coordinates of the runway threshold. The car drives in a straight
    -- line to this point and stops there with "Arrived at destination".
    -- The virtual node has hotzone="" so it never triggers a premature stop.
    if is_backtaxi and depart_arrive == 1 and #t_node > 0 then
...
function determine_pos_on_segment ( in_heading, in_x, in_z, in_type )
  ...
  for i = 1, #t_segment do
      if ( in_type == "gate" or in_type == "misc" ) then
          l_within_sight, l_intersect_x, l_intersect_z, l_dist_to_intersect = compute_intersection (in_type, in_heading, in_x, in_z, i)
      ...
      if ( in_type == "tie_down" or in_type == "hangar" ) then   
          ...
          l_within_sight, l_tangent_dist, l_tangent_x, l_tangent_z, l_goto_node = compute_tangent_dist( in_heading, in_x, in_z, i)
  ...
  if l_ret_node_dist < 9999 and l_ret_intersect_dist < 9999 then
      if l_ret_intersect_dist <= l_ret_node_dist then l_decision = 1 else l_decision = 2 end
  ...
  if l_in_sight then 
    ...
    if ( ( l_tangent_x >= l_node1_x and l_tangent_x <= l_node2_x ) or
         ...
         ( ( l_tangent_z >= l_node1_z and l_tangent_z <= l_node2_z ) or
           ( l_tangent_z >= l_node2_z and l_tangent_z <= l_node1_z ) ) then     
...
function compute_intersection (in_type, in_heading, in_x, in_z, in_idx)
  ...
  if ( ( l_intersect_x >= l_node1_x and l_intersect_x <= l_node2_x ) or
      ( l_intersect_x >= l_node2_x and l_intersect_x <= l_node1_x ) ) and
              ...
              ( l_intersect_z >= l_node2_z and l_intersect_z <= l_node1_z ) ) then   
  else
      return false, 0, 0, 0
  end
  ...
  local l_in_sight, _, l_dist_to_intersect = chk_line_of_sight (l_heading, 70, 70, in_x, in_z, l_intersect_x, l_intersect_z)  
 
  if l_in_sight == false and ( in_type == "tie_down" or in_type == "hangar" ) then
        l_heading = add_delta_clockwise( in_heading, 180, 1)
        l_in_sight, _, l_dist_to_intersect = chk_line_of_sight (l_heading, 70, 70, in_x, in_z, l_intersect_x, l_intersect_z)      
  end
...
function add_new_taxinode_segment(in_segment_index, in_x, in_z, in_intersect_dist)
  ...
  t_taxinode[l_idx].f_value = nil; t_taxinode[l_idx].g_value = nil; t_taxinode[l_idx].h_value = nil
  t_taxinode[l_idx].parent = nil; t_taxinode[l_idx].cost = nil; t_taxinode[l_idx].heading = nil  
  l_new_node = l_idx - 1
  ...
  if string.find(t_taxinode[t_segment[in_segment_index].Node1 + 1].Segment, ",") then 
      ...
      t_segment[l_new_segment].Heading, t_segment[l_new_segment].Dist = heading_n_dist( t_taxinode[t_segment[in_segment_index].Node1 + 1].x,
                                                                                        t_taxinode[t_segment[in_segment_index].Node1 + 1].z,
                                                                                        in_x, in_z)
      t_taxinode[t_segment[in_segment_index].Node1+1].Segment = t_taxinode[t_segment[in_segment_index].Node1+1].Segment..","..tostring(l_new_segment)
  ...
  if string.find(t_taxinode[t_segment[in_segment_index].Node2 + 1].Segment, ",") then   
      ...
      t_segment[l_new_segment].Size = t_segment[in_segment_index].Size
      t_segment[l_new_segment].Hotzone = t_segment[in_segment_index].Hotzone
      t_segment[l_new_segment].Heading, t_segment[l_new_segment].Dist = heading_n_dist( in_x, in_z,
                                                                                        t_taxinode[t_segment[in_segment_index].Node2 + 1].x,
                                                                                        t_taxinode[t_segment[in_segment_index].Node2 + 1].z)
      t_taxinode[t_segment[in_segment_index].Node2+1].Segment = t_taxinode[t_segment[in_segment_index].Node2+1].Segment..","..tostring(l_new_segment)
...
function auto_assign_gate()
    ...
    for l_index = 1, #t_gate  do
        if  string.match(t_gate[l_index].Types, Aircraft_Type) == Aircraft_Type or 
                            ( Aircraft_Type == "0" and string.match(t_gate[l_index].Types, "7" ) ) then
...
function load_config()
  ...
  if l_file == nil then return "-2" end
  ...
  repeat
    ...
    if l_line then
      ...
      elseif l_str1 == "car_type_fmcar" then
        if l_str2 == "Ferrari" then car_type_fmcar = "Ferrari"
        elseif l_str2 == "Van" then car_type_fmcar = "Van"
        elseif l_str2 == "Truck" then car_type_fmcar = "Truck"
        elseif l_str2 == "Auto" then car_type_fmcar = "Auto"
        end
  ...
  if l_aircraft_type ~= "" then return "" else return "-1" end
end
...
function save_config()
  ...
  if l_file == nil then return "-4" end
