...
function determine_runway_node()
  ...
  -- VER1.7 Coussini 2026 : Second pass for runways still unassigned (Node == 0)
  -- These are back-taxi runways where the threshold has no direct taxiway connection
  -- (e.g. MDPC RWY 27 : seuil only reachable via the runway itself from the other end)
  -- We assign them the closest runway node already in use, even if it is a duplicate.
  -- The A* will then route through the runway segments to reach the threshold.
  for l_idx = 1, #t_runway do
      if t_runway[l_idx].Node == -1 then  -- VER1.7 : -1 = non assigné
          local l_min_dist = 99999
          local l_best_node = -1
          local l_node_x, l_node_z = 0, 0
          -- Search all runway nodes (including duplicates) for the closest one
          for _, l_rwy_node in ipairs(t_runway_node) do
              _, l_node_z = 0, 0
              local l_nx = t_taxinode[l_rwy_node+1].x
              local l_nz = t_taxinode[l_rwy_node+1].z
              local _, l_curr_dist = heading_n_dist( l_nx, l_nz,
                                                     t_runway[l_idx].x,
                                                     t_runway[l_idx].z )
              if l_curr_dist < l_min_dist then
                  l_min_dist = l_curr_dist
                  l_best_node = l_rwy_node
              end
          end
          if l_best_node >= 0 then  -- VER1.7 : 0 est un node_id valide
              t_runway[l_idx].Node = l_best_node
              t_taxinode[l_best_node+1].Runway = t_runway[l_idx].ID
              logMsg("FollowMe VER1.7 : back-taxi runway " .. t_runway[l_idx].ID ..
                     " assigned to node " .. l_best_node ..
                     " (dist=" .. string.format("%.1f", l_min_dist) .. "m)")
          end
      end
  end
...
function determine_possible_routes()
    ...
    if depart_arrive == 1 then
        ...
        -- VER1.10 : improved back-taxi detection
        -- A runway is back-taxi ONLY if no runway node within 200m has a taxiway exit.
        -- This fixes false positives like CYHU RWY 06L where node K is only 39m away.
        -- MDPC RWY 27 is a true back-taxi because no taxiway exit exists within 200m.
        is_backtaxi = false
        local l_has_nearby_exit = false
        local l_endpt_x2 = t_taxinode[l_endpt_node+1].x
        local l_endpt_z2 = t_taxinode[l_endpt_node+1].z
        local l_backtaxi_radius = 200  -- metres in local XZ coordinates

        for l_index = 1, #t_taxinode do
            -- Check if this node is a runway node within 200m of the threshold
            if t_taxinode[l_index].Type == "runway" then
                local _, l_near_dist = heading_n_dist(l_endpt_x2, l_endpt_z2,
                                                       t_taxinode[l_index].x,
                                                       t_taxinode[l_index].z)
                if l_near_dist <= l_backtaxi_radius then
                    -- Does this nearby runway node have a taxiway exit ?
                    for l_seg = 1, #t_segment do
                        local l_node_id = l_index - 1  -- taxinode is 1-based, node_id is 0-based
                        if ( t_segment[l_seg].Node1 == l_node_id or
                             t_segment[l_seg].Node2 == l_node_id ) and
                           t_segment[l_seg].Type ~= "runway" then
                            l_has_nearby_exit = true
                            break
                        end
                    end
                end
            end
            if l_has_nearby_exit then break end
        end

        if not l_has_nearby_exit then
            is_backtaxi = true
            logMsg("FollowMe VER1.10 : back-taxi detected for RWY " .. depart_runway ..
                   " (no taxiway exit within 200m of threshold)")
        else
            logMsg("FollowMe VER1.10 : RWY " .. depart_runway ..
                   " has taxiway exit within 200m — normal departure (not back-taxi)")
        end
    else
