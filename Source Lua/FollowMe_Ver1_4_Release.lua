...
--    VER1.4 Coussini 2026 : X-Plane 12 compatibility
--                           Delta-time correction for physics
--                           Auto Toliss/standard aircraft detection
--                           Cancel Fix, diamond marker, LIGHT PROP default
--    ---------------------------------------------------------------------------------
...
function handle_plugin_window()
    ...
    if holder_wnd and toggle_window == true then
        toggle_window = false
        logMsg("FM DEBUG : toggle_window processed - followme_wnd=" .. tostring(followme_wnd ~= nil))
        if followme_wnd ~= nil then
            logMsg("FM DEBUG : calling hide_window()")
            hide_window()
        else
            logMsg("FM DEBUG : calling show_window()")
            window_is_open = true
            show_window()
        end
    end
...
function hide_window()
  if followme_wnd ~= nil then
    ...
    if holder_wnd ~= nil then
        float_wnd_bring_to_front(holder_wnd)
        ignore_next_release = 2  -- block next 2 parasitic releases after bring_to_front
        logMsg("FM DEBUG : hide_window called - ignore_next_release set to 2")
    end
  end
end
...
function build_window(wnd, x, y)
  ...
  l_changed, l_newval = imgui.Checkbox("##Get from FMS", get_from_FMS)
  if l_changed then
      get_from_FMS = l_newval
      if not get_from_FMS then
          depart_runway = ""
      end  
  end  
  
  imgui.SameLine()  
  imgui.TextUnformatted("Get from FMS")  
...
function build_holder(wnd, x, y)
  ...
  if imgui.IsMouseReleased(0) then
      logMsg("FM DEBUG : IsMouseReleased received in build_holder - followme_wnd=" .. tostring(followme_wnd ~= nil) .. " holder_drag=" .. tostring(holder_drag) .. " ignore=" .. tostring(ignore_next_release))
      if ignore_next_release > 0 then
          ignore_next_release = ignore_next_release - 1
          logMsg("FM DEBUG : IsMouseReleased ignored - lock=" .. tostring(ignore_next_release))
      elseif fm_gear1_gnd ~= 0 and fm_gear2_gnd ~= 0 then
          if holder_drag < 2 then
              toggle_window = true
              logMsg("FM DEBUG : toggle_window set to TRUE")
          else
              logMsg("FM DEBUG : toggle_window NOT set - holder_drag=" .. tostring(holder_drag))
          end
          holder_drag = 0
      else
          logMsg("FM DEBUG : IsMouseReleased ignored - aircraft in flight (gear off ground)")
      end  
  end  
end
...
function load_config()
  ...
  repeat     
    ...
    if l_line then 
      ...
      elseif l_str1 == "get_from_FMS" then
        get_from_FMS = (l_str2 == "1")
...
function save_config()
  ...
  l_content = l_content .. "get_from_FMS" .. "\t" .. (get_from_FMS and "1" or "") .. "\n"
