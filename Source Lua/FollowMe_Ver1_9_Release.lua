...
function determine_possible_routes()
    ...
    if depart_arrive == 1 then
        ...
        -- VER1.8 : detect back-taxi — runway node has ONLY runway neighbours (no taxiway exit)
        -- In that case the car must ride the runway all the way to the threshold
        is_backtaxi = false
        local l_has_taxiway_exit = false
        for l_index = 1, #t_segment do
            if ( t_segment[l_index].Node1 == l_endpt_node or
                 t_segment[l_index].Node2 == l_endpt_node ) and
               t_segment[l_index].Type ~= "runway" then
        ...
        if not l_has_taxiway_exit then
            is_backtaxi = true
            logMsg("FollowMe VER1.8 : back-taxi detected for RWY " .. depart_runway ..
                   " (no taxiway exit at threshold — will ride runway to threshold)")
        end
    else
