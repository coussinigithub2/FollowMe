...
function check_SimBrief()
    ...
    if simbrief_id == "" then
        ...
        logMsg("No SimBrief ID provided, normal use without SimBrief")
        return
    end

    sb_fetch_status = "LOADING"
    logMsg("SimBrief recovery...")
    ...
    if code ~= 200 or not response_body or #response_body == 0 then
        ...
        logMsg("SimBrief error (HTTP " .. tostring(code) .. "). The plugin continues without SimBrief.")
        return
    end
    ...
    if sb_origin_icao == "" or sb_dest_icao == "" then
        ...
        logMsg("SimBrief does not contain valid data. The plugin continues normally.")
        return
    end

    sb_fetch_status = "OK"
    logMsg("SimBrief OK - " .. sb_origin_icao .. " → " .. sb_dest_icao)
...
function apply_simbrief_runway()
    ...
    if sb_fetch_status ~= "OK" then
        logMsg("No SimBrief runway application (status = " .. sb_fetch_status .. ")")
        return
    end
    ...
    if l_rwy == "" then
        logMsg("SimBrief did not provide a track for this mode.")
        return
    end
    ...
    for i = 1, #t_runway do
        if t_runway[i].ID == l_rwy then
            depart_runway = l_rwy
            logMsg("Runway SimBrief applied : " .. l_rwy)
            return
        end
    end

    -- Si la piste n'existe pas → ne rien bloquer
    logMsg("The SimBrief runway (" .. l_rwy .. ") does not exist at this airport. Ignored.")
end
...
function move_car( in_dist, in_car_in_sight, in_car_is_behind)
    ...
    elseif in_dist >= avg_dist_from_plane and in_dist < max_dist_from_plane and in_car_in_sight then
        if car_speed > 0 then
          if fm_gnd_spd > car_speed then     
              car_accel =   (fm_gnd_spd - car_speed) / elapsed_time
...
function unload_rampstart()
    if rampstart_instance[0] ~= nil then XPLM.XPLMDestroyInstance(rampstart_instance[0]) end
...
function handle_plugin_window()
    ...
    if ( fm_gear1_gnd == 0 and fm_gear2_gnd == 0 ) and fm_new_flight > 1  then
        ...
        if holder_wnd and toggle_window == true then
          window_is_open = not window_is_open
  ...
  if prepare_kill_objects == true then
      ...
      if kill_is_manual then
          ...
          rampstart_chg  = true   -- redraw ramp marker with restored gate
      else
          if flightstart == 9999 then
              unload_rampstart()
          end
...
function show_window()
  window_first_access = true
  followme_wnd = float_wnd_create(400, 500, 2, true)
...
function build_window(wnd, x, y)
  ...
  if l_changed then
      -- Limit to 10 numeric characters
  ...
  if sb_fetch_status == "OK" then
      if get_from_SimBrief and sb_airport_mismatch then
          imgui.PushStyleColor(imgui.constant.Col.Text, 0xFF0000FF)  -- rouge
          imgui.TextUnformatted("No SimBrief data for the current airport")
          imgui.PopStyleColor()
      else
          imgui.PushStyleColor(imgui.constant.Col.Text, 0xFF00FF00)  -- vert
          imgui.TextUnformatted("SimBrief Runways  Dep:" .. sb_runway_takeoff .. "  Arr:" .. sb_runway_landing)
          imgui.PopStyleColor()
      end
  elseif sb_fetch_status == "LOADING" then
      imgui.PushStyleColor(imgui.constant.Col.Text, 0xFF00FFFF)  -- cyan
  ...
  local l_y2 = 16
  for i= 1, 3 do
    imgui.SetCursorPosY(458 + l_y2 * i)
    imgui.SetCursorPosX(20)
...
function closed_window(wnd) end
...
function build_holder(wnd, x, y)
  ...
  imgui.SetCursorPos( l_win_width / 2 - l_text_width / 2,  l_win_height / 2 - l_text_height / 2)
  imgui.TextUnformatted(l_text)
  imgui.DrawList_AddCircle(l_win_width / 2, l_win_height / 2, 12, 0xFFFFFFFF)

  if imgui.IsMouseDragging(0) then   
      if holder_drag == 0 then
          holder_drag = 1
          prev_mouse_Y = MOUSE_Y
      end  
      if holder_drag > 0 then
          l_diff = (MOUSE_Y - prev_mouse_Y)
          if math.abs(l_diff) > 0 then
              Win_Y = Win_Y -  l_diff
              prev_mouse_Y = MOUSE_Y
              float_wnd_set_position(holder_wnd, screen_width - Holder_len,  Win_Y)  
              holder_drag = 3
...
function load_config()
  ...
  l_file = io.open( syspath .. "Output/preferences/FollowMe.prf", "r")    
  ...
  repeat     
    ...
    if l_line then 
      ...
      if l_str1 == "holder_ypos" then
        Win_Y = tonumber( l_str2 )
...
function save_config()
  ...
  l_content = l_content .. "holder_ypos" .. "\t" .. tostring(Win_Y) .. "\n"
  
  -- VER1.5 : Save SimBrief settings (replaces FMS)
  l_content = l_content .. "get_from_SimBrief" .. "\t" .. (get_from_SimBrief and "1" or "") .. "\n"
  l_content = l_content .. "simbrief_id" .. "\t" .. simbrief_id .. "\n"
  
  l_content = l_content .. "show_path" .. "\t" .. (show_path and "1" or "") .. "\n"
  l_content = l_content .. "show_rampstart" .. "\t" .. (show_rampstart and "1" or "") .. "\n"
  l_content = l_content .. PLANE_ICAO .. "\t" .. Aircraft_Type .. "\n"     
  l_content = l_content .. "random_gate" .. "\t" .. (random_gate and "1" or "") .. "\n"
  l_content = l_content .. "vol" .. "\t" .. tostring(vol) .. "\n"    
  l_content = l_content .. "speed_limiter" .. "\t" .. (speed_limiter and "1" or "") .. "\n"
  
  if car_type_fmcar == "Ferrari" then l_content = l_content .. "car_type_fmcar\tFerrari\n" 
  elseif car_type_fmcar == "Van" then l_content = l_content .. "car_type_fmcar\tVan\n" 
  elseif car_type_fmcar == "Truck" then l_content = l_content .. "car_type_fmcar\tTruck\n" 
  elseif car_type_fmcar == "Auto" then l_content = l_content .. "car_type_fmcar\tAuto\n"     
  end 
  ...
  if l_file ~= nil then
    repeat     
      ...
      if l_line then 
        l_str1, l_str2 = string.match(l_line,  "([%p%a%d]+)%S-(.*)" )
        if l_str1 ~= "holder_ypos" and 
                l_str1 ~= "get_from_SimBrief" and
                l_str1 ~= "get_from_FMS" and         -- VER1.5 : remove legacy FMS key if present
                l_str1 ~= "simbrief_id" and
                l_str1 ~= "show_path" and 
                l_str1 ~= PLANE_ICAO and 
                l_str1 ~= "random_gate" and
                l_str1 ~= "vol" and
                l_str1 ~= "speed_limiter" and
                l_str1 ~= "car_type_fmcar" then 
          l_content = l_content .. l_line .. "\n"                       
        end  
      end      
    until not l_line
    l_file:close()   
  ...
  l_file = io.open( syspath .. "Output/preferences/FollowMe.prf", "w")    
